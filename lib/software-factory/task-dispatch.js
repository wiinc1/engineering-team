...
async function dispatchTaskToSpecialist(task, options = {}) {
  const baseSpecialist = options.targetSpecialist || resolveTaskSpecialist(task.type);
  const specialist = baseSpecialist
    ? normalizeSpecialistForDelegation({
      targetSpecialist: baseSpecialist,
      ownerAgentId: options.ownerAgentId || task.ownerAgentId || task.owner?.id || task.owner?.actor_id,
      engineerTier: options.engineerTier || task.engineerTier || task.engineer_tier,
    })
    : null;
  if (!specialist) {
