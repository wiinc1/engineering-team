  ...
  return Promise.race([
    ...
    new Promise((_, reject) => {
      timer = setTimeout(() => {
        ...
        reject(new LangGraphRuntimeError('langgraph_checkpoint_unavailable', {
...
function initialState(input, config, clock) {
  ...
  return validateFactoryState({
    ...
    threadId,
    lifecycleNode: null,
    completedNodes: input.state?.completedNodes || [],
    artifacts: input.state?.artifacts || [],
    decisions: input.state?.decisions || [],
    attempt: input.state?.attempt || 0,
    updatedAt: new Date(clock.now()).toISOString(),
  }, { maxBytes: config.maxStateBytes });
}
function graphRunnableConfig(state, signal) {
  // LangGraph reserves the empty checkpoint namespace for the root graph. The
  // application-level "factory" namespace remains durable in factory_threads.
  return {
    configurable: { thread_id: state.threadId, checkpoint_ns: '' },
    ...(signal ? { signal } : {}),
  };
}
function beginLeaseHeartbeat(registry, input, onFailure) {
  ...
  const renew = () => {
    renewal = renewal.then(async () => {
      ...
      try {
        await timeout(registry.renewLease(input), intervalMs);
      } catch (error) {
        failed = true;
        try { onFailure(asRuntimeError(error)); } catch {}
      }
    });
  };
...
function runtimeDependencies(options) {
  ...
  return { checkpointer, clock, config, graph, logger, metrics, ownsPool, pool, registry, sharedPool };
}
...
class LangGraphRuntime {
    ...
    try {
      const operation = (async () => {
        ...
        try {
          ...
          await this.updateCompletionStatus(state.tenantId, state.threadId, runnableConfig);
    ...
    try {
      const operation = (async () => {
        ...
        try {
          ...
          await this.updateCompletionStatus(input.tenantId, threadId, runnableConfig);
          return validateFactoryState(projectFactoryState(result), { maxBytes: this.config.maxStateBytes });
        } catch (error) {
  ...
  async updateCompletionStatus(tenantId, threadId, runnableConfig) {
    ...
    await this.updateLeaseStatus(tenantId, threadId, paused ? 'paused' : 'completed');
...
module.exports = { createLangGraphRuntime, graphRunnableConfig, initialState, timeout };
