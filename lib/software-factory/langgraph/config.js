...
function runtimeConfig(input = {}, env = process.env) {
  ...
  const ownershipEpoch = String(input.ownershipEpoch ?? env.LANGGRAPH_OWNERSHIP_EPOCH ?? '').trim() || null;
  ...
  if (production && enabled && !UUID_PATTERN.test(ownershipEpoch || '')) throw invalidConfig('ownership_epoch_required');
