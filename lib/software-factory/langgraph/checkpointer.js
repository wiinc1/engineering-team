...
class GuardedPostgresSaver extends PostgresSaver {
  ...
  async putWrites(config, writes, taskId) {
    ...
    if (checkpointId && checkpointId !== record.last_checkpoint_id) {
      ({ binding, record } = await this.assertBound(config));
      const key = `${binding.threadId}:${checkpointId}`;
      let pending = this.pendingAcceptance.get(key);
      if (!pending && checkpointId !== record.last_checkpoint_id) {
        const prefix = `${binding.threadId}:`;
        const active = [...this.pendingAcceptance.entries()]
          .filter(([candidate]) => candidate.startsWith(prefix)).map(([, promise]) => promise);
        if (active.length) {
          await Promise.all(active);
          await Promise.resolve();
          pending = this.pendingAcceptance.get(key);
        }
      }
      if (!pending) {
    ...
    const accepted = checkpointId && (checkpointId === record.last_checkpoint_id
