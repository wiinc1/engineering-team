...
async function decide(deps, input) {
  ...
  const record = pending?.interrupt_id === input.interruptId
    ? pending
    : await deps.registry.interruptById?.(input.tenantId, input.threadId, input.interruptId);
  if (!record) throw new LangGraphRuntimeError('langgraph_interrupt_not_found');
  if (!authorizedForInterrupt({ authorizedRoles: record.authorized_roles }, input.roles)) {
    deps.metrics.increment('langgraph_interrupt_decisions_total', { type: record.interrupt_type, outcome: 'failed', code: 'langgraph_decision_forbidden' });
  ...
  return claim.replay ? replayResult(claim.interrupt) : finishDecision(deps, input, record, decision, idempotencyKey);
}
