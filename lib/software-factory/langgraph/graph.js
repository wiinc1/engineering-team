...
const UPDATE_KEYS = new Set([
  'lifecycleNode', 'completedNodes', 'artifacts', 'decisions', 'attempt', 'updatedAt',
  'lifecycleStatus', 'qaOutcome', 'qaAttempts', 'terminalReason', 'nodeAttempts', 'childRuns',
]);
...
function compileFactoryGraph(options = {}) {
  ...
  graph.addEdge(START, options.entryNode || options.nodes[0].name);
  if (options.transitions) {
    const names = new Set(options.nodes.map((node) => node.name));
    for (const node of options.nodes) {
      const transition = options.transitions[node.name];
      if (!transition) throw new TypeError(`Missing LangGraph transition for ${node.name}`);
      if (typeof transition === 'string') {
        if (transition !== END && !names.has(transition)) throw new TypeError(`Unknown LangGraph target: ${transition}`);
        graph.addEdge(node.name, transition);
        continue;
      }
      if (typeof transition.route !== 'function' || !transition.targets || typeof transition.targets !== 'object') {
        throw new TypeError(`Invalid conditional transition for ${node.name}`);
      }
      for (const target of Object.values(transition.targets)) {
        if (target !== END && !names.has(target)) throw new TypeError(`Unknown LangGraph target: ${target}`);
      }
      graph.addConditionalEdges(node.name, transition.route, transition.targets);
    }
  } else {
