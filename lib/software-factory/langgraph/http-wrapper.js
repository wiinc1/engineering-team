...
const { authorized, createLangGraphHttpHandler } = require('./http');
...
const { createLifecycleRuntime } = require('./lifecycle-runtime');
const { createProductionLifecyclePorts } = require('./production-ports');
const { createCanonicalLifecycleServices } = require('../../task-platform/langgraph-lifecycle-services');
...
function runtimeEnabled(options) {
  const value = options.ffLangGraphRuntime ?? process.env.FF_LANGGRAPH_RUNTIME;
...
function createRuntimeLoader(bundle, options) {
  ...
  return async () => {
    if (!runtimePromise) runtimePromise = (async () => {
      ...
      const enabled = options.ffLangGraphRuntime ?? process.env.FF_LANGGRAPH_RUNTIME;
      const active = runtimeEnabled(options);
      const lifecycleHandlers = options.lifecycleHandlers
        || (options.lifecycleHandlerFactory
          ? options.lifecycleHandlerFactory({ store: bundle.store })
          : null);
      const lifecycleServices = options.lifecycleServices
        || (lifecycleHandlers
          ? createCanonicalLifecycleServices({ store: bundle.store, handlers: lifecycleHandlers })
          : null);
      const lifecyclePorts = options.lifecyclePorts
        || (lifecycleServices ? createProductionLifecyclePorts(lifecycleServices) : null);
      if (active && !lifecyclePorts) {
        throw new LangGraphRuntimeError('langgraph_configuration_invalid', {
          safeDetails: { reason: 'lifecycle_ports_missing' },
        });
      }
      ...
      // A disabled deployment may expose storage health before lifecycle ports
      // are installed. Once enabled, only the complete production lifecycle is
      // accepted; a placeholder graph can never process a factory run.
      const runtime = lifecyclePorts
        ? createLifecycleRuntime({ ...runtimeOptions, ports: lifecyclePorts })
        : createLangGraphRuntime({ ...runtimeOptions, nodes: [{ name: 'disabled_runtime', execute: () => ({}) }] });
      await runtime.setup();
      return runtime;
    })();
...
function createLangGraphRouteWrapper(bundle, options = {}, helpers = {}) {
  if (runtimeEnabled(options) && !options.langGraphRuntime && !options.lifecyclePorts
    && !options.lifecycleServices && !options.lifecycleHandlers && !options.lifecycleHandlerFactory) {
    throw new LangGraphRuntimeError('langgraph_configuration_invalid', {
      safeDetails: { reason: 'lifecycle_ports_missing' },
    });
  }
...
module.exports = {
  createLangGraphRouteWrapper, langGraphOperatorRoute, langGraphRoute, mutationEnabled,
  normalizeRoutePath, operatorStatus, runtimeEnabled,
};
