...
function extraProvidersFromEnv(env = process.env) {
  ...
  for (const [name, spec] of Object.entries(parsed)) {
    ...
    assertRunnerAllowlisted(runner, { providerName });
    extras[providerName] = {
      name: providerName,
      runner,
...
function assertRunnerAllowlisted(runner, { providerName = 'extra' } = {}) {
  const normalized = String(runner || '').replace(/\\/g, '/').trim();
  if (isFixtureDelegationRunner(normalized)) {
    throw createProviderError(
      PROVIDER_UNKNOWN,
      `Provider ${providerName} cannot use the fixture specialist runner`,
    );
  }
  const allowed = /(?:^|\s)node\s+(?:['"]?)(?:\.\/)?(?:[\w./-]*\/)?scripts\/[A-Za-z0-9._-]+-specialist-runner\.js['"]?\s*$/.test(normalized)
    || /(?:^|\/)scripts\/[A-Za-z0-9._-]+-specialist-runner\.js$/.test(normalized);
  if (!allowed) {
    throw createProviderError(
      PROVIDER_UNKNOWN,
      `Provider ${providerName} runner must be a repo scripts/*-specialist-runner.js command`,
      { runner: normalized },
    );
  }
  return true;
...
function applySelectedSpecialistRuntimeEnv(env = process.env, options = {}) {
  const resolved = resolveSpecialistDelegationRunner({ env, options });
  if (env.SPECIALIST_RUNTIME_PROVIDER == null || env.SPECIALIST_RUNTIME_PROVIDER === '') {
    env.SPECIALIST_RUNTIME_PROVIDER = resolved.name;
  }
  if (!Object.prototype.hasOwnProperty.call(env, 'SPECIALIST_DELEGATION_RUNNER')) {
    env.SPECIALIST_DELEGATION_RUNNER = resolved.runner;
  }
  if (!env.FF_REAL_SPECIALIST_DELEGATION) env.FF_REAL_SPECIALIST_DELEGATION = 'true';
  return { env, resolved };
}
