...
function buildForgeReadyApprovedContract(taskId, options = {}) {
  const affectsUi = options.affectsUi === true;
...
function buildForgeReadyLifecycleEvents(tenantId, taskId, approvedContract) {
  return [
    {
      tenantId, taskId, eventType: 'task.architect_engineer_assignment_recorded', actorType: 'agent', actorId: 'architect',
      idempotencyKey: `architect-assign:${taskId}:v1`,
      payload: {
        contract_version: approvedContract.version,
        engineer_tier: 'Sr',
        assignee: 'engineer-sr',
        tier_rationale: 'Local forge smoke seeds architect assignment before forge dispatch.',
    ...
    },
    {
      ...
      idempotencyKey: `assign:${taskId}:engineer-sr`, payload: { assignee: 'engineer-sr' },
    },
  ];
}
...
async function persistForgeReadyTaskEvents(store, tenantId, taskId, createdPayload, approvedContract) {
  const events = [
    ...
    ...buildForgeReadyLifecycleEvents(tenantId, taskId, approvedContract),
  ];
  const results = [];
...
async function seedForgeLocalSmokeTask(options = {}) {
  ...
  const { createdPayload, approvedContract } = buildForgeReadyApprovedContract(taskId, {
    affectsUi: options.affectsUi === true,
  });
