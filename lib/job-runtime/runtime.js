...
class JobRuntime {
  ...
  async start() {
    ...
    try {
      ...
      this.runner = await this.options.adapter.start(this.options.taskList, this.options.config, this.events);
  ...
  async pruneRetention() {
    if (typeof this.options.registry.pruneTerminalBefore !== 'function') return 0;
    const cutoff = new Date(this.options.clock.now() - this.options.config.retentionDays * 24 * 60 * 60 * 1_000);
    try {
      const count = await this.options.registry.pruneTerminalBefore(cutoff, this.options.config.retentionBatch);
      this.options.metrics.increment('job_runtime_registry_pruned_total', {}, count);
      return count;
    } catch (error) {
      this.options.metrics.increment('job_runtime_retention_failure_total');
      this.options.logger.error('job_runtime_retention_failed', { error: sanitizedError(error) });
      return 0;
    }
  }

  scheduleRetention() {
    if (this.retentionTimer || typeof this.options.timers.setInterval !== 'function') return;
    this.retentionTimer = this.options.timers.setInterval(
      () => { void this.pruneRetention(); },
      this.options.config.retentionIntervalMs,
    );
  ...
  clearRetention() {
    ...
    this.options.timers.clearInterval?.(this.retentionTimer);
    this.retentionTimer = null;
  }
