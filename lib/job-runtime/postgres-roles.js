...
function grantStatements(roles) {
  ...
  return [
    ...
    `GRANT USAGE, CREATE ON SCHEMA ${GRAPHILE_WORKER_SCHEMA}, ${JOB_RUNTIME_SCHEMA} TO ${migrator}`,
    ...
    `GRANT USAGE ON SCHEMA ${GRAPHILE_WORKER_SCHEMA}, ${JOB_RUNTIME_SCHEMA} TO ${producer}`,
    ...
    `GRANT USAGE ON SCHEMA ${GRAPHILE_WORKER_SCHEMA}, ${JOB_RUNTIME_SCHEMA} TO ${worker}`,
...
async function verifyJobRuntimePrivileges(pool) {
  ...
  if (!privileges?.graphile_usage || !privileges?.registry_usage || !privileges?.registry_access || !privileges?.effect_access) {
