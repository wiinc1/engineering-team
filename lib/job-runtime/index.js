...
const { createJobOperatorService } = require('./operator-service');
...
function createInfrastructureCore(options) {
  ...
  const ownershipGuard = options.ownershipGuard || (config.ownershipEpoch
    ? createDatabaseOwnershipGuard(sharedPool, { scope: 'jobs', engine: 'graphile', epoch: config.ownershipEpoch })
    : null);
  ...
  const common = { catalog, validator, registry, adapter, logger, metrics, timers, ownershipGuard, clock: options.clock || { now: Date.now } };
...
function createJobRuntimeInfrastructure(options = {}) {
  ...
  return Object.freeze({
    ...
    pool: sharedPool, runtimePool: pool, port, producers, registry, runtime, scheduler, adapter: core.adapter,
  });
}
