...
class DeliveryRegistry {
  ...
  async completeOperatorAction(input) {
    ...
    try {
      await client.query('BEGIN');
      const delivery = await client.query(
        `UPDATE ${TABLE} SET status = $4::varchar, operator_version = operator_version + 1,
           last_error_code = CASE WHEN $4::text = $5::text THEN last_error_code ELSE NULL END,
           cancelled_at = CASE WHEN $4::text = $6::text THEN NOW() ELSE cancelled_at END,
           scheduled_for = CASE WHEN $4::text = $7::text THEN NOW() ELSE scheduled_for END,
           updated_at = NOW()
