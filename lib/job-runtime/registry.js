...
function pendingValues(input) {
  return [
    ...
    input.queue, input.maxAttempts, input.priority, input.canonicalResourceType,
...
class DeliveryRegistry {
  ...
  async createPending(input) {
    const result = await this.pool.query(
      `INSERT INTO ${TABLE} (
        ...
        task_name, payload_version, catalog_version, named_queue, max_attempts,
  ...
  async operationalMetrics() {
    const result = await this.pool.query(
      `SELECT
         COUNT(*) FILTER (WHERE status IN ($1, $2, $3))::integer AS queue_depth,
         COALESCE(EXTRACT(EPOCH FROM (NOW() - MIN(scheduled_for)
           FILTER (WHERE status IN ($1, $2, $3)))), 0)::double precision AS oldest_age_seconds
       FROM ${TABLE}`,
