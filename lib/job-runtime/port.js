...
class JobRuntimePort {
  ...
  async enqueueValidated(context, request) {
    ...
    const schedule = { ...schedulingPolicy(definition), runAt: scheduledFor };
    return this.persistAndEnqueue({ context, request, canonical, definition, envelope, validated, scheduledFor, schedule, key });
  }
