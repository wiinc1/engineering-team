...
function normalizeRegistrationMode(value = 'invite-only') {
  const mode = String(value || 'invite-only').trim().toLowerCase();
...
function buildRegistrationPolicy(options = {}) {
  ...
  const registrationMode = normalizeRegistrationMode(options.registrationMode || process.env.AUTH_REGISTRATION_MODE || 'invite-only');
