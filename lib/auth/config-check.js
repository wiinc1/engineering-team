...
function buildAuthDiagnostics(env = process.env) {
  ...
  return {
    ...
    requiredInternalBootstrapVarsPresent: Object.fromEntries(
      REQUIRED_INTERNAL_BOOTSTRAP_VARS.map((name) => [
        name,
        name === 'AUTH_PRODUCTION_AUTH_STRATEGY' ? productionAuthStrategy === 'internal-bootstrap' : hasValue(env, name),
      ])
    ),
    requiredRegistrationVarsPresent: Object.fromEntries(
      REQUIRED_REGISTRATION_VARS.map((name) => [
        name,
        name === 'AUTH_PRODUCTION_AUTH_STRATEGY'
          ? productionAuthStrategy === 'registration'
          : hasValue(env, name),
    ...
    browserRegistrationStrategyVarsPresent: Object.fromEntries(
      BROWSER_REGISTRATION_STRATEGY_VARS.map((name) => [name, hasValue(env, name)])
    ),
    registrationProductionRulesSatisfied: {
