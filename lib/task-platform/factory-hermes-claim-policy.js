...
function hermesEvaluationContext(options) {
  const env = options.env || process.env;
  const role = resolveHermesClaimRole(env);
  const hermesUrl = String(options.hermesUrl != null ? options.hermesUrl : (env.HERMES_BASE_URL || '')).trim();
  return {
    role,
    hermesUrl,
    classification: classifyHermesEndpoint(hermesUrl),
    liveLike: isLiveLikeProofProfile(options, env),
    decision: { ...HERMES_CLAIM_DECISION, role },
  };
}

function requiredHermesFailure(context, code, note) {
  return {
    ok: false,
    required: true,
    role: context.role,
    classification: context.classification,
    hermesUrl: context.hermesUrl || null,
    claimTopology: true,
    code,
...
function evaluateRequiredHermes(context) {
  if (context.classification === 'unset') {
    return requiredHermesFailure(
      context,
      HERMES_CLAIM_ERROR_CODES.REQUIRED_UNAVAILABLE,
  ...
  if (context.classification === 'mock') {
    const note = context.liveLike
      ? `Live factory claim cannot use hermes-mock at ${context.hermesUrl} (GitLab #272). `
        + 'Use a real HERMES_BASE_URL or de-scope with FACTORY_HERMES_CLAIM_ROLE=non-critical.'
      : `Hermes mock at ${context.hermesUrl} cannot satisfy required Hermes claim topology.`;
    return requiredHermesFailure(context, HERMES_CLAIM_ERROR_CODES.MOCK_FORBIDDEN, note);
  }
  return {
    ok: true, required: true, role: context.role, classification: context.classification,
    hermesUrl: context.hermesUrl, claimTopology: true, code: null, decision: context.decision,
    note: 'Real Hermes endpoint configured for required claim topology.',
  };
}

function evaluateOptionalHermes(context) {
  let note = 'Hermes URL present but not required for factory claim topology (GitLab #272).';
  if (context.classification === 'mock') {
    note = 'hermes-mock is optional non-claim smoke only; not part of factory-of-record claim topology (GitLab #272).';
  } else if (context.classification === 'unset') {
    note = 'Hermes unset is allowed: non-critical for Simple factory claims (GitLab #272).';
  }
  return {
    ok: true, required: false, role: context.role, classification: context.classification,
    hermesUrl: context.hermesUrl || null, claimTopology: false, code: null,
    decision: context.decision, note,
  };
}
...
function evaluateHermesClaimTopology(options = {}) {
  const context = hermesEvaluationContext(options);
  return context.role === 'required' ? evaluateRequiredHermes(context) : evaluateOptionalHermes(context);
}
