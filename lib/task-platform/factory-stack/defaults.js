...
function specialistRuntimeServiceEnv() {
  const {
    resolveProviderName,
    resolveSpecialistDelegationRunner,
  } = require('../../software-factory/specialist-runtime-provider');
  const resolved = resolveSpecialistDelegationRunner({ env: process.env });
  return {
    SPECIALIST_RUNTIME_PROVIDER: resolveProviderName({ env: process.env }),
    GROK_BIN: process.env.GROK_BIN || 'grok',
    SPECIALIST_DELEGATION_RUNNER: process.env.SPECIALIST_DELEGATION_RUNNER || resolved.runner,
    SPECIALIST_RUNTIME_RUNNER_TIMEOUT_MS: process.env.SPECIALIST_RUNTIME_RUNNER_TIMEOUT_MS || '300000',
...
function coreServiceEnv(apiPort, uiPort) {
  ...
  return {
    ...
    FF_GITLAB_INTAKE_NORMALIZER: process.env.FF_GITLAB_INTAKE_NORMALIZER || 'true',
    FF_GITLAB_INTAKE_PROJECT_BOOTSTRAP: process.env.FF_GITLAB_INTAKE_PROJECT_BOOTSTRAP || 'true',
    FORGE_INTAKE_PROVIDER: process.env.FORGE_INTAKE_PROVIDER || 'gitlab',
    GITLAB_WEBHOOK_SECRET: process.env.GITLAB_WEBHOOK_SECRET || 'golden-path-local-webhook-secret',
    FF_REAL_SPECIALIST_DELEGATION: 'true',
    GOLDEN_PATH_OPENCLAW_POST_APPROVAL_ARTIFACTS: 'true',
    GOLDEN_PATH_OPENCLAW_ARCHITECT_ENGINEER_ASSIGNMENT: 'true',
    ...
    OPENCLAW_BASE_URL: defaultOpenclawUrl(),
    FORGE_SERVICE_TOKEN: forgeServiceToken(),
    FORGEADAPTER_SERVICE_TOKEN: forgeadapterServiceToken(),
    FORGEADAPTER_BASE_URL: `http://127.0.0.1:${DEFAULT_PORTS.forgeadapter}`,
...
function buildServiceEnv(overrides = {}) {
  return {
    ...coreServiceEnv(DEFAULT_PORTS.api, DEFAULT_PORTS.ui),
    ...specialistRuntimeServiceEnv(),
    ...overrides,
  };
}
