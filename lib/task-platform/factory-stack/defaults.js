...
function buildServiceEnv(overrides = {}) {
  ...
  const providerName = resolveProviderName({ env: process.env });
  const resolved = resolveSpecialistDelegationRunner({ env: process.env });
  const runner = process.env.SPECIALIST_DELEGATION_RUNNER || resolved.runner;
  ...
  return {
    ...
    SPECIALIST_RUNTIME_PROVIDER: providerName,
