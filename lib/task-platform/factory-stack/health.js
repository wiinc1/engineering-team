...
async function collectHealthReport({
  ...
  // Hermes mock probe is never required for stack health under default de-scope (#272).
  const hermesMockOptional = {
    ...hermesMock,
  ...
  // When Hermes is forced required, missing/mock Hermes fails claim topology.
  const hermesClaimOk = hermesClaim.ok === true;
  const claimTopologyOk = claimOk && hermesClaimOk;
