...
const DEFAULT_DELEGATION_RUNNER = GROK_SPECIALIST_RUNNER;
...
function resolveAgentDelegationRunner(options = {}, env = process.env) {
  ...
  if (
    ...
    assertNonFixtureDelegationRunner(resolved.runner, options, env);
...
function resolveAgentDelegationEnv(options = {}, env = process.env) {
  ...
  return {
    ...
    SPECIALIST_RUNTIME_PROVIDER: resolveProviderName({ env, options }),
...
function buildFactoryExecutionContractBody({
  requirements,
  templateTier = 'Simple',
  changeKind = '',
  changedFiles = [],
  agentDrivenPhase1 = false,
  agentDrivenPhases = false,
  actorId = null,
  requireHumanPmArchitectReview,
} = {}) {
  ...
  } = require('./factory-human-pm-architect');
  const { contract } = applyFactoryHumanPmArchitectContractFields(base, {
    agentDrivenPhase1,
    agentDrivenPhases,
    actorId,
    requireHumanPmArchitectReview,
...
function createFactoryAgentCoordinator(options = {}) {
  ...
  return createSpecialistCoordinator({
    ...
    delegationRunnerTimeoutMs: options.delegationRunnerTimeoutMs,
    runnerEnv: options.openclawTimeoutSec ? {
      OPENCLAW_DELEGATION_TIMEOUT_SEC: String(options.openclawTimeoutSec),
    } : undefined,
  });
}
