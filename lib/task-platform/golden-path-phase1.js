...
async function apiSend(ctx, route, method, roles, body) {
  ...
  const { resolveWorkflowRoute } = require('./golden-path-shared');
  const response = await ctx.fetchImpl(buildUrl(ctx.baseUrl, resolveWorkflowRoute(route, ctx)), {
    method,
    headers: {
...
async function runGoldenPathPhase1(options = {}) {
  ...
  if (ctx.agentDrivenPhase1) {
    Object.assign(process.env, resolveAgentDelegationEnv({ openclawUrl: ctx.openclawUrl, collectRealEvidence: options.collectRealEvidence === true, requireRealEvidence: options.requireRealEvidence === true, agentDrivenPhases: options.agentDrivenPhases === true }));
  }
  ...
  const contractBody = ctx.agentDrivenPhase1
    ? buildFactoryExecutionContractBody({
      ...
      actorId: ctx.actorId,
      requireHumanPmArchitectReview: options.requireHumanPmArchitectReview,
    })
  ...
  // Q6 / #275 / #276: factory agent-driven contracts require human PM/Architect acceptance
  // before Simple auto-approval. PM-refinement may record a contract without those fields,
  // and createExecutionContractDraft historically dropped them — record explicitly.
  if (ctx.agentDrivenPhase1 || options.agentDrivenPhases === true || contractBody.require_human_pm_architect_review === true) {
    ...
    phase1: {
      ...
      contract: {
        ...
        autoApprovalApproved: approvalData?.autoApproval?.approved_by_policy
          ...
          ?? true,
        requireHumanPmArchitectReview: contractBody.require_human_pm_architect_review === true,
        agentProposals: contractBody.agent_proposals || null,
        humanReviewsRecorded: Boolean(
          contractBody.human_reviews?.pm && contractBody.human_reviews?.architect,
        ),
      },
      architectSpec: {
