...
async function runProjectionCatchUp(ctx, label) {
  const mode = ctx.persistDir ? 'local_file_sync' : 'remote_postgres';
  if (mode === 'local_file_sync') {
    return {
      label,
      mode,
      processed: 0,
      failed: 0,
      skipped: true,
      reason: 'File-backed local stack uses synchronous projection; bounded worker not required.',
    };
  }
  ...
  if (!fs.existsSync(scriptPath)) {
    return {
      ...
      failed: 0,
      skipped: true,
      reason: 'Projection worker script unavailable; operator must run bounded catch-up manually.',
    };
  }

  const baseDir = ctx.persistDir || process.cwd();
  const { stdout } = await execFileAsync(process.execPath, [scriptPath, baseDir, '25'], {
    cwd: process.cwd(),
    env: { ...process.env, AUDIT_STORE_BACKEND: process.env.AUDIT_STORE_BACKEND || 'postgres' },
  });
