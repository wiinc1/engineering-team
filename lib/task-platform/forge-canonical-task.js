...
} = require('../audit/execution-contract-ux-dispatch');
...
} = require('../audit/forge-issue-intake-shared');
...
function buildAuditSummary(taskId, state = {}, history = []) {
  ...
  const rawRequirements = resolveOperatorIntakeRequirements(history);
  const intakeDraft = !!(
  ...
  return {
    task_id: taskId,
    title: normalizeText(resolveIntakeDraftTitle(history, taskId)) || taskId,
...
function isDraftOrUnapprovedContract(summary, contract) {
  if (!contract || contract.status !== 'approved') return true;
  const stage = String(summary.current_stage || '').toUpperCase();
  // Intake drafts stay in DRAFT through post-approval gates; forge readiness uses dispatch gates instead.
  if (stage === 'DRAFT' && summary.intake_draft) return false;
  return stage === 'DRAFT';
}
...
function evaluateForgeExecutionReadiness({ taskId, state = null, history = [] }) {
  ...
  } = require('../audit/execution-contract-architect-dispatch');
  const dispatchReadiness = augmentDispatchReadinessWithArchitectGate(
    augmentDispatchReadinessWithUxGate(
      evaluateExecutionContractDispatchReadiness({
        contract,
    ...
    {
      contract,
      architectEngineerAssignment: findLatestArchitectEngineerAssignment(history, contract.version),
      history,
    },
  );
...
function resolveForgeCanonicalFields({
  ...
  const hasExplicitForgeDispatch = Object.keys(forgeDispatch).length > 0;
  const approvedIntakeDefaults = !hasExplicitForgeDispatch
    && contract.status === 'approved'
    && summary.intake_draft
    ? { projectId: 'engineering-team', targetRepo: 'wiinc1/engineering-team' }
    : null;

  return {
    projectId: normalizeText(forgeDispatch.project_id ?? forgeDispatch.projectId)
      || normalizeText(taskPlatformProjectId)
      || approvedIntakeDefaults?.projectId
      || null,
    domain: normalizeText(forgeDispatch.domain) || 'workflow',
    targetRepo: normalizeText(forgeDispatch.target_repo ?? forgeDispatch.targetRepo)
      || approvedIntakeDefaults?.targetRepo
      || null,
