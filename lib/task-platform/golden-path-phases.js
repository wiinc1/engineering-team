...
async function runGoldenPathPhase2(ctx, evidence, options = {}) {
  ...
  const skipForge = options.skipForgeSeed === true || options.skipForgePhases === true
    || process.env.STAGING_SKIP_FORGE_SEED === 'true'
    || process.env.STAGING_SKIP_FORGE_PHASES === 'true';
  // Local live/fixture factory proof skips forgeadapter; only load harness when needed.
  ...
  if (!skipForge) {
    ...
    api.seed = { ok: true, skipped: true, reason: 'skip_forge_seed' };
    api.readiness = { ok: true, skipped: true, reason: 'skip_forge_seed', status: 200, body: { ready: true, skipped: true } };
    api.start = {
      ok: true,
      skipped: true,
    ...
    api.stack = { skipped: true, reason: 'skip_forge_seed', source: 'skip_forge_seed', baseUrl: null, faToken: null };
    api.stackHealth = { ok: true, skipped: true, reason: 'skip_forge_seed' };
    api.runtimeAfterStart = { executionState: 'skipped_forge_seed' };
  }
...
async function runGoldenPathPhase3(ctx, phase2Result, evidence, options = {}) {
  ...
  if (shouldSkipForgePhases(options, phase2Result)) {
    api.forgeReject = {
      ok: true,
      skipped: true,
      reason: 'skip_forge_seed',
    ...
    api.runtimeAfterReject = { executionState: 'skipped_forge_seed', workflowState: 'skipped_forge_seed' };
  } else {
...
async function runGoldenPathPhase4(ctx, phase2Result, phase3Result, evidence, options = {}) {
  ...
  let runtimeAfterResume = { body: { executionState: 'skipped_forge_seed' } };
  if (shouldSkipForgePhases(options, phase2Result)) {
    api.forgeResume = {
      ok: true,
      skipped: true,
...
async function runGoldenPathPhase5(ctx, phase2Result, evidence, options = {}) {
  ...
  let runtime = {
    response: { status: 200 },
    body: { executionState: 'skipped_forge_seed', workflowState: 'skipped_forge_seed' },
  };
  if (shouldSkipForgePhases(options, phase2Result)) {
    for (const gate of ['qa', 'architect', 'pm']) {
      api[`gate_${gate}`] = { ok: true, skipped: true, reason: 'skip_forge_seed' };
    }
    api.complete = {
      ok: true,
      skipped: true,
