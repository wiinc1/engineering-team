...
async function runDelegationSmoke() {
  try {
...
async function runProjectionCatchUp(ctx, maxEvents = 25) {
  ...
  if (!fs.existsSync(scriptPath)) {
    return { skipped: true, reason: 'projection_script_missing' };
  }
...
async function latestContractCoverageAttempt(ctx, taskId) {
  ...
  const latest = entries.find((entry) => entry.event_type === 'task.contract_coverage_audit_submitted');
...
async function waitForContractCoverageHistoryEvent(ctx, taskId, eventType) {
  for (let attempt = 0; attempt < 24; attempt += 1) {
    const projection = await runProjectionCatchUp(ctx).catch(() => ({ processed: 0 }));
    const history = await apiGet(ctx, `/tasks/${encodeURIComponent(taskId)}/history`, ['reader', 'admin']);
    const entries = history.body?.items || history.body?.data?.items || [];
    const found = entries.some((entry) => entry.event_type === eventType);
    if (found && (projection.processed > 0 || attempt >= 8)) {
      return;
    }
    await new Promise((resolve) => setTimeout(resolve, 250));
  }
  throw new Error(`${eventType} did not project for ${taskId}`);
}
...
async function completeContractCoverageAuditGate(ctx, taskId, options = {}) {
  ...
  if (currentStage !== 'CONTRACT_COVERAGE_AUDIT' && currentStage !== 'QA_TESTING') {
    ...
    if (coveredAttempt !== implementationVersion) {
      ...
      await waitForContractCoverageHistoryEvent(ctx, taskId, 'task.contract_coverage_audit_submitted');
    } else {
  ...
  if (!validate.ok && validate.status === 409) {
    ...
    await waitForContractCoverageHistoryEvent(ctx, taskId, 'task.contract_coverage_audit_validated');
  }
...
async function advancePilotTaskToCloseReview(ctx, taskId) {
  let currentStage = await readPilotTaskStage(ctx, taskId) || 'SRE_MONITORING';

  if (currentStage === 'PM_CLOSE_REVIEW') {
    return { currentStage, transitions: [] };
  }

  const transitions = [];
  if (currentStage === 'QA_TESTING') {
    transitions.push({ from: 'QA_TESTING', to: 'SRE_MONITORING' });
    currentStage = 'SRE_MONITORING';
  }
  if (currentStage === 'SRE_MONITORING') {
    transitions.push({ from: 'SRE_MONITORING', to: 'PM_CLOSE_REVIEW' });
  } else {
...
async function runGoldenPathPhase2(ctx, evidence, options = {}) {
  ...
  const persistDir = ctx.persistDir || evidence.phase1?.persistDir || evidence.phase0?.localBaseDir;
  ...
  try {
    api.stack = await forge.harness.startLocalStack({
      etDir,
      taskId: forgeTaskId,
      sharedToken: options.forgeServiceToken || DEFAULT_FORGE_SERVICE_TOKEN,
      faToken: options.forgeAdapterToken || DEFAULT_FORGE_ADAPTER_TOKEN,
      tenantId: ctx.tenantId,
      affectsUi: false,
    });
  } finally {
  ...
  const runtime = await forge.service.requestJson(`${api.stack.baseUrl}/tasks/${forgeTaskId}/runtime`, {
  ...
  const commitSha = options.implementationCommitSha || buildGoldenPathCommitSha('golden-path-impl');
  const prUrl = options.prUrl || 'https://github.com/wiinc1/engineering-team/pull/271';
  ...
  const implementationStages = new Set(['IN_PROGRESS', 'IMPLEMENTATION', 'CONTRACT_COVERAGE_AUDIT', 'QA_TESTING', 'SRE_MONITORING', 'PM_CLOSE_REVIEW']);
...
async function runGoldenPathPhase3(ctx, phase2Result, evidence, options = {}) {
  ...
  if (etTaskId && pilotState?.latest_qa_outcome === 'fail') {
    api.stageAdvanceForQa = await advancePilotTaskToQaTesting(ctx, etTaskId, {
    ...
    if (etTaskId && api.stageAdvanceForQa && api.stageAdvanceForQa.skipped !== true) {
      await waitForPilotTaskStage(ctx, etTaskId, 'QA_TESTING');
      await runProjectionCatchUp(ctx).catch(() => {});
    }
  }
  if (
...
async function runGoldenPathPhase4(ctx, phase2Result, phase3Result, evidence, options = {}) {
  ...
  const fixCommitSha = options.fixCommitSha || buildGoldenPathCommitSha('golden-path-fix');
...
function validationSubprocessEnv() {
  ...
  delete env.AUDIT_STORE_BACKEND;
...
async function tryVercelProductionDeploy(options = {}) {
  if (options.skipVercelDeploy) {
    return { ok: false, skipped: true, reason: 'skip_vercel_flag' };
  }
  try {
    const { stdout, stderr } = await execFileAsync('npx', ['vercel', 'deploy', '--prod', '--yes'], {
      cwd: options.cwd || process.cwd(),
      env: process.env,
      timeout: 600000,
    });
    const urlMatch = stdout.match(/https:\/\/[^\s]+/g);
    const productionUrl = urlMatch?.[urlMatch.length - 1] || null;
    const idMatch = stdout.match(/dpl_[A-Za-z0-9]+/);
    return {
      ok: true,
      stdout,
      stderr,
...
async function runGoldenPathPhase6(ctx, evidence, options = {}) {
  ...
  api.vercel = await tryVercelProductionDeploy(options);
  const productionUrl = api.vercel.productionUrl
    || options.productionUrl
    || evidence.vercel?.productionUrl
    || 'https://engineering-team-zeta.vercel.app';
  ...
  if (etTaskId && stage === 'SRE_MONITORING' && !pilotState?.sre_approved_at) {
    ...
    api.sreMonitoring = {
      ...
      waiver: true,
      stage,
      reason: 'Local pilot advanced to PM_CLOSE_REVIEW in Phase 5 without SRE_MONITORING window; production replay must execute GP-026 from SRE_MONITORING.',
    };
  ...
  return {
    ...
    api: {
      ...
      vercel: {
        ok: api.vercel.ok,
        skipped: api.vercel.skipped === true,
        productionUrl,
        deploymentId: api.vercel.deploymentId || null,
        stderr: api.vercel.stderr ? String(api.vercel.stderr).slice(0, 500) : null,
      },
...
async function runGoldenPathPhase5(ctx, phase2Result, evidence) {
  const { stack, forge, forgeTaskId } = phase2Result;
  const etTaskId = evidence.engineeringTeam?.taskId;
  const api = {};

  api.stageAdvanceForCloseReview = etTaskId
    ? await advancePilotTaskToCloseReview(ctx, etTaskId)
    : { skipped: true };
  ...
  if (etTaskId && (!api.pmClose.ok || !api.architectClose.ok)) {
    throw new Error(
...
async function runGoldenPathPhases(options = {}) {
  ...
  const ctx = {
    ...
    persistDir: options.persistDir || evidence.phase1?.persistDir || evidence.phase0?.localBaseDir,
  };
  ...
  if (phaseOnly || (fromPhase <= 6 && toPhase >= 6 && fromPhase > 5)) {
    ...
    evidence.vercel = {
      ...(evidence.vercel || {}),
      deploymentId: phase6Result.api.vercel.deploymentId,
      productionUrl: phase6Result.api.vercel.productionUrl,
    };
  ...
  try {
    if (fromPhase <= 2 && toPhase >= 2) {
      ...
      evidence.forgeadapter = {
        ...
        startJobId: phase2Result.api.startJobId,
        completeJobId: null,
    ...
    if (fromPhase <= 5 && toPhase >= 5) {
      ...
      const phase5Result = await runGoldenPathPhase5(ctx, phase2Result, evidence);
