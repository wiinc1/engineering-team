...
const { applyTrustedSimpleMerge, finalizeTrustedSimpleCloseEvidence, runTrustedSimpleCleanQa } = require('./trusted-simple-merge');
const { assertRealPhase6AutoMerge } = require('./golden-path-phase6-auto-merge-proof');
const { refreshPhase6ReleaseEvidenceAfterAutoMerge } = require('./golden-path-release-evidence-refresh');
const { syncSrePullRequest } = require('./golden-path-sre-pr-sync');
const {
  DEFAULT_PILOT_PR_NUMBER, DEFAULT_PILOT_PR_URL, assertRealImplementationEvidence,
  assertRealPhase6Evidence, isRealEvidenceRequired, resolvePhase2ImplementationEvidence,
  resolvePhase4FixEvidence, resolvePhase5ImplementationEvidence, resolvePhase6ReleaseEvidence,
} = require('./golden-path-real-evidence');
const { parseGitHubPullRequestUrl, prepareGoldenPathRealEvidence, shouldCollectGoldenPathRealEvidence } = require('./golden-path-real-evidence-collector');
const { fixtureDelegationFailures } = require('./golden-path-real-evidence-preflight');
const { goldenPathArchitectCloseRationale, goldenPathCloseSummary, goldenPathHumanCloseRationale, goldenPathHumanCloseSummary, goldenPathPmCloseRationale, goldenPathPrTitle, goldenPathTaskCloseReason, realCodeChangeSreApproval } = require('./golden-path-messages');
const {
...
} = require('./golden-path-forge-skip');
const {
  partitionForgeSteps,
...
const { runDeployValidation } = require('./golden-path-validation');
  ...
  const env = {
    ...
    SPECIALIST_DELEGATION_RUNNER: options.delegationRunner
      ...
      || require('../software-factory/specialist-runtime-provider')
        .resolveSpecialistDelegationRunner({ env: process.env }).runner,
  };
  try {
    const { stdout, stderr } = await execFileAsync('npm', ['run', 'test:delegation:live-smoke'], {
      cwd: process.cwd(),
      env,
      timeout: 120000,
    });
    const sessionMatch = stdout.match(/sessionId["':\s]+([A-Za-z0-9._:-]+)/i)
      || stdout.match(/specialist-delegation-[0-9a-f-]{36}/i);
    const sessionId = sessionMatch
      ? (sessionMatch[1] || sessionMatch[0] || null)
      : null;
    return {
      ok: true,
      stdout,
...
async function advancePilotTaskToImplementation(ctx, taskId) {
  ...
  // READY is the post-approval workflow stage used by factory proof after
  // Execution Contract approval (Simple auto-approval path). Advance to IMPLEMENTATION.
  const readyPath = [{ from: 'READY', to: 'IMPLEMENTATION' }];
...
async function runGoldenPathPhase2(ctx, evidence, options = {}) {
  ...
  const templateTierForForge = options.templateTier
    || evidence.engineeringTeam?.templateTier
    || evidence.phase1?.contract?.templateTier
    || 'Simple';
  const forgeSkipDecision = resolveForgeSkipDecision({
    ...options,
    templateTier: templateTierForForge,
  });
  const forgePolicyRecord = forgeSkipDecision.record;
  const skipForge = forgeSkipDecision.skip;
  // Local Simple claim path may skip forgeadapter under #273 policy; non-Simple fails closed.
  const forge = skipForge ? null : requireForgeHarness();
  const api = {};
  ...
  if (!skipForge) {
    ...
    Object.assign(api, buildSkippedForgeSeedApi(forgePolicyRecord));
  }
  ...
  if (options.agentDrivenPhases) {
    Object.assign(process.env, resolveAgentDelegationEnv({ openclawUrl: options.openclawUrl, hermesUrl: options.hermesUrl, requireRealEvidence: isRealEvidenceRequired(options) }));
  }
  ...
  const implementation = resolvePhase2ImplementationEvidence(
    evidence, options, agentImplementer, buildGoldenPathCommitSha('golden-path-impl'),
  );
  const { branchName, commitSha, prUrl, prNumber } = implementation;
  ...
  // GP-012/014 are ET/GitHub work that still runs under Simple forge skip;
  // only forgeadapter GPs (009/010/011/016/018/020) go to stepsSkipped.
  const forgeStepPartition = partitionForgeSteps({
    skipped: skipForge,
    includeGp013: api.delegationSmoke?.ok === true,
    includeGp012: api.pilotAgentsSeed?.ok !== false,
    includeGp014: Boolean(
      implementation?.commitSha
      || implementation?.prUrl
      || api.engineerSubmission?.ok
      || agentImplementer
    ),
    phase: 'phase2',
  });
  const steps = forgeStepPartition.completed;
  const stepsSkipped = forgeStepPartition.skipped;

  return {
    steps,
    stepsSkipped,
    forgePolicy: forgePolicyRecord,
    forgeTaskId,
    forgeAuditDir,
    stack: api.stack,
    forge,
    api: {
      ...
      stackSource: api.stack?.source || null,
      startJobId: api.start?.job?.id || api.start?.job?.jobId || api.start?.action?.jobId || null,
      ...
      implementation: { branchName, commitSha, prUrl, prNumber },
    },
  };
}
...
async function runGoldenPathPhase3(ctx, phase2Result, evidence, options = {}) {
  ...
  if (etTaskId && pilotState?.latest_qa_outcome === 'pass') {
    ...
    const priorRunId = qaResults.body?.latestFailed?.runId
      ...
      || null;
    const earlySkip = shouldSkipForgePhases(options, phase2Result);
    const earlySteps = partitionPhaseStepsWithForgePolicy({
      steps: ['GP-015', 'GP-016'],
      forgeStepsInPhase: ['GP-016'],
      skipped: earlySkip,
    });
    return {
      steps: earlySteps.steps,
      stepsSkipped: earlySteps.stepsSkipped,
      api: {
  ...
  const trustedQa = await runTrustedSimpleCleanQa({ ctx, etTaskId, evidence, options, phase2Result, recordQaResult: recordQaResultWithCatchUp }); if (trustedQa.result) return trustedQa.result;
  const trustedQaAgent = trustedQa.agent;
  if (
    etTaskId
  ...
  if (etTaskId && pilotState?.latest_qa_outcome === 'fail') {
    ...
    let qaAgent = trustedQaAgent;
    if (options.agentDrivenPhases && !qaAgent) {
  ...
  const skipForgePhase3 = shouldSkipForgePhases(options, phase2Result);
  if (skipForgePhase3) {
    api.forgeReject = buildSkippedForgeJob('skip_forge_seed', phase2Result?.forgePolicy || null);
    api.runtimeAfterReject = buildSkippedForgeRuntime('skipped_forge_seed', phase2Result?.forgePolicy || null).body;
  } else {
  ...
  const phase3Steps = partitionPhaseStepsWithForgePolicy({
    steps: ['GP-015', 'GP-016'],
    forgeStepsInPhase: ['GP-016'],
    skipped: skipForgePhase3,
  });
  return {
    steps: phase3Steps.steps,
    stepsSkipped: phase3Steps.stepsSkipped,
    api: {
      qaFail: {
...
async function runGoldenPathPhase4(ctx, phase2Result, phase3Result, evidence, options = {}) {
  ...
  if (
    ...
    && Number(pilotStateAtStart?.implementation_submission_version || 0) >= (options.trustedSimpleClose === true ? 1 : 2)
  ) {
    const earlySkip = shouldSkipForgePhases(options, phase2Result);
    const earlySteps = partitionPhaseStepsWithForgePolicy({
      steps: ['GP-017', 'GP-018', 'GP-019'],
      forgeStepsInPhase: ['GP-018'],
      skipped: earlySkip,
    });
    return {
      steps: earlySteps.steps,
      ...
      api: {
        ...
        fixCommitSha: pilotStateAtStart.implementation_commit_sha || phase2Result.api.implementation?.commitSha,
      },
    };
  }
  ...
  if (options.agentDrivenPhases) {
    fixAgent = await runImplementerFixAgentPhase(ctx, {
      ...
      requirements: evidence.engineeringTeam?.requirements,
      openclawUrl: options.openclawUrl,
      branchName: options.fixBranchName || options.branchName || options.branch || evidence.github?.branchName, commitSha: options.fixCommitSha,
      prUrl: options.fixPrUrl || phase2Result.api.implementation.prUrl, requireRealEvidence: isRealEvidenceRequired(options),
      trustedDelivery: options.trustedDelivery === true, trustedSimpleClose: options.trustedSimpleClose === true, proofProfile: options.proofProfile,
    });
  }
  api.readmeFix = addReadmeGoldenPathMarker();
  const fixProof = resolvePhase4FixEvidence(evidence, options, phase2Result.api.implementation, fixAgent, buildGoldenPathCommitSha('golden-path-fix')), fixCommitSha = fixProof.commitSha;
  if (fixAgent) {
    api.fixAgent = {
      delegated: fixAgent.delegated,
      sessionId: fixAgent.sessionId,
      agentId: fixAgent.agentId,
      branchName: fixProof.branchName || null, commitSha: fixCommitSha, prUrl: fixProof.prUrl || phase2Result.api.implementation.prUrl,
    };
  }
  ...
  let runtimeAfterResume = buildSkippedForgeRuntime();
  const skipForgePhase4 = shouldSkipForgePhases(options, phase2Result);
  if (skipForgePhase4) {
    api.forgeResume = buildSkippedForgeJob('skip_forge_seed', phase2Result?.forgePolicy || null);
    runtimeAfterResume = buildSkippedForgeRuntime('skipped_forge_seed', phase2Result?.forgePolicy || null);
  } else {
    api.forgeResume = await runForgeControlAction(stack, forge, `/tasks/${forgeTaskId}/resume`);
    runtimeAfterResume = await forge.service.requestJson(`${stack.baseUrl}/tasks/${forgeTaskId}/runtime`, {
      headers: forge.harness.serviceHeaders(stack.faToken),
    });
  }
  ...
  const phase4Steps = partitionPhaseStepsWithForgePolicy({
    steps: ['GP-017', 'GP-018', 'GP-019'],
    forgeStepsInPhase: ['GP-018'],
    skipped: skipForgePhase4,
  });
  return {
    steps: phase4Steps.steps,
    stepsSkipped: phase4Steps.stepsSkipped,
    api: {
...
function resolveOperatorAppUrl(ctx, evidence, options = {}) {
  return options.operatorUrl
    ...
    || evidence.operatorHosted?.productionUrl || evidence.deployment?.url
    || ctx?.baseUrl
...
async function resolveSreApprovalPayload(ctx, evidence, options, { etTaskId, operatorUrl, mergeCommitSha }) {
  ...
  if (isRealEvidenceRequired(options)) return realCodeChangeSreApproval(operatorUrl, mergeCommitSha);
  return {
...
async function runGoldenPathPhase6(ctx, evidence, options = {}) { assertRealEvidenceModeOptions(options);
  const etTaskId = evidence.engineeringTeam?.taskId;
  ...
  const releaseProof = resolvePhase6ReleaseEvidence(evidence, options);
  let mergeCommitSha = releaseProof.mergeCommitSha;
  const { prUrl, prNumber } = releaseProof, prRepository = parseGitHubPullRequestUrl(prUrl)?.repository || null, repository = options.repository || options.ciRepository || evidence.github?.repository || prRepository || null, hasPrTarget = Number.isFinite(Number(prNumber)) && Boolean(prUrl);
  if (isRealEvidenceRequired(options) && (!hasPrTarget || (prRepository && repository !== prRepository))) throw new Error(!hasPrTarget ? 'GP-022 real-evidence auto-merge requires an actual pull request target' : 'GP-022 real-evidence repository must match pull request URL');

  api.autoMerge = hasPrTarget ? await mergePullRequestWhenReady({
    repository,
    prNumber,
    prUrl,
    ...
    fetchImpl: options.fetchImpl || ctx.fetchImpl,
  }) : { ok: true, skipped: true, simulated: false, reason: 'missing_pr_target', prNumber: null, prUrl: null };
  if (!api.autoMerge.ok && !api.autoMerge.skipped) {
    throw new Error(`GP-022 auto-merge failed: ${JSON.stringify(api.autoMerge)}`);
  }
  if (isRealEvidenceRequired(options) || options.trustedSimpleClose === true) assertRealPhase6AutoMerge(api.autoMerge);
  if (api.autoMerge.mergeCommitSha) mergeCommitSha = api.autoMerge.mergeCommitSha;
  finalizeTrustedSimpleCloseEvidence(evidence, options, api.autoMerge, mergeCommitSha);

  const prSyncedAt = new Date().toISOString();
  api.prSync = etTaskId && hasPrTarget
    ? await apiSend(ctx, `/tasks/${encodeURIComponent(etTaskId)}/events`, 'POST', ['admin'], {
      ...
      payload: {
        pr_number: prNumber,
        pr_title: goldenPathPrTitle(isRealEvidenceRequired(options)),
        state: 'closed',
        pr_state: 'merged',
        pr_merged: true,
        ...
        linked_prs: [{
          number: prNumber,
          url: prUrl,
          title: goldenPathPrTitle(isRealEvidenceRequired(options)),
          repository,
          state: 'merged',
          merged: true,
    ...
    : { ok: true, skipped: true, reason: hasPrTarget ? 'missing_task' : 'missing_pr_target' };
  if (etTaskId && hasPrTarget && !api.prSync.ok) {
  ...
  await refreshPhase6ReleaseEvidenceAfterAutoMerge({ api, ctx, evidence, options, mergeCommitSha });
  ...
  api.supabase = { skipped: true, reason: 'cloud_supabase_not_in_factory_stack' };
  ...
  if (pilotState?.sre_approved_at || evidence.phase5?.api?.sreMonitoring?.approve?.ok) {
    ...
    if (!sreDetailBeforeStart.windowStartedAt) {
      if (hasPrTarget) await waitForMergedLinkedPr(ctx, etTaskId);
      api.sreStart = await apiSend(ctx, `/tasks/${encodeURIComponent(etTaskId)}/sre-monitoring/start`, 'POST', ['sre', 'admin'], {
        ...
        evidence: ['Golden path phase 6 deploy validation passed.', hasPrTarget ? `PR ${prUrl} merged.` : 'No PR target was provided for local smoke evidence.'],
      });
      if (!api.sreStart.ok) {
  ...
  if (etTaskId && pilotState?.closed) {
    ...
    api.humanClose = await apiSend(
      ...
      {
        outcome: 'approve',
        summary: goldenPathHumanCloseSummary(isRealEvidenceRequired(options)),
        rationale: goldenPathHumanCloseRationale(isRealEvidenceRequired(options)),
  ...
  api.taskClosed = etTaskId && !pilotState?.closed
    ? await apiSend(ctx, `/tasks/${encodeURIComponent(etTaskId)}/events`, 'POST', ['admin'], {
      ...
      payload: {
        reason: goldenPathTaskCloseReason(isRealEvidenceRequired(options)),
  ...
  return {
    ...
    api: {
      ...
      mergeCommitSha,
      prUrl, prNumber, repository,
      validation: api.validation,
      releaseEvidence: api.releaseEvidence,
      deploy: api.deploy,
...
async function runGoldenPathPhase5(ctx, phase2Result, evidence, options = {}) {
  ...
  if (taskAlreadyClosed && evidence.forgeadapter?.completeJobId) {
    const closedSkip = shouldSkipForgePhases(options, phase2Result);
    const closedSteps = partitionPhaseStepsWithForgePolicy({
      steps: ['GP-020', 'GP-021', 'GP-026'],
      forgeStepsInPhase: ['GP-020'],
      skipped: closedSkip,
    });
    return {
      steps: closedSteps.steps,
      stepsSkipped: closedSteps.stepsSkipped,
      api: {
  ...
  const operatorUrl = resolveOperatorAppUrl(ctx, evidence, options);
  const { mergeCommitSha, prUrl, prNumber } = resolvePhase5ImplementationEvidence(evidence, options), prRepository = parseGitHubPullRequestUrl(prUrl)?.repository || null, repository = options.repository || options.ciRepository || evidence.github?.repository || prRepository || null;
  const hasPrTarget = Number.isFinite(Number(prNumber)) && Boolean(prUrl); if (isRealEvidenceRequired(options) && prRepository && repository !== prRepository) throw new Error('GP-022 real-evidence repository must match pull request URL');
  pilotState = etTaskId ? await readPilotTaskState(ctx, etTaskId) : pilotState;
  ...
  if (etTaskId && taskAlreadyClosed) {
    ...
    steps.push('GP-026');
  } else if (etTaskId && !sreAlreadyApproved && (hasPrTarget || !isRealEvidenceRequired(options))) {
    api.prSyncForSre = await syncSrePullRequest({
      ctx, apiSend, etTaskId, hasPrTarget, prNumber, prUrl, mergeCommitSha, repository,
      title: goldenPathPrTitle(isRealEvidenceRequired(options)),
    });
    ...
    api.prSyncForSre = { ok: true, skipped: true, reason: 'missing_pr_target' };
  }
  if (etTaskId && !sreAlreadyApproved) {
    const sreDetailBeforeStart = extractSreMonitoringDetail(
      (await apiGet(ctx, `/tasks/${encodeURIComponent(etTaskId)}/detail`, ['reader', 'admin'])).body,
    );
    if (!sreDetailBeforeStart.windowStartedAt) {
      // Local live/fixture proof often has no real PR target; only wait when one was synced.
      if (hasPrTarget) {
      ...
      api.sreStart = await apiSend(ctx, `/tasks/${encodeURIComponent(etTaskId)}/sre-monitoring/start`, 'POST', ['sre', 'admin'], {
        ...
        deploymentStatus: 'success',
        evidence: [
          'Golden path phase 5 deploy validation window.',
          `Operator URL ${operatorUrl}.`,
          hasPrTarget ? `PR ${prUrl} merged.` : 'No PR target was provided for local live/fixture proof.',
        ],
      });
      if (!api.sreStart.ok) {
        throw new Error(`GP-026 SRE start failed (${api.sreStart.status}): ${JSON.stringify(api.sreStart.body)}`);
  ...
  let runtime = buildSkippedForgeRuntime();
  const skipForgePhase5 = shouldSkipForgePhases(options, phase2Result);
  if (skipForgePhase5) {
    for (const gate of ['qa', 'architect', 'pm']) {
      api[`gate_${gate}`] = { ok: true, skipped: true, reason: 'skip_forge_seed', forgePolicy: phase2Result?.forgePolicy || null };
    }
    api.complete = buildSkippedForgeJob('skip_forge_seed', phase2Result?.forgePolicy || null);
  } else {
    ...
    if (forgeAlreadyCompleted) {
      ...
      runtime = runtimeBeforeClose.response.status === 200
        ? runtimeBeforeClose
  ...
  if (etTaskId && taskAlreadyClosed) {
    ...
    api.pmClose = etTaskId
      ? await apiSend(ctx, `/tasks/${encodeURIComponent(etTaskId)}/close-review/cancellation-recommendation`, 'POST', ['pm', 'admin'], {
        summary: goldenPathCloseSummary(isRealEvidenceRequired(options)),
        rationale: goldenPathPmCloseRationale(isRealEvidenceRequired(options)),
    ...
    api.architectClose = etTaskId
      ? await apiSend(ctx, `/tasks/${encodeURIComponent(etTaskId)}/close-review/cancellation-recommendation`, 'POST', ['architect', 'admin'], {
        ...
        rationale: goldenPathArchitectCloseRationale(isRealEvidenceRequired(options)),
        recommendation: 'close',
      })
      : { ok: false, skipped: true };
  }
  ...
  const phase5Steps = partitionPhaseStepsWithForgePolicy({
    steps,
    forgeStepsInPhase: ['GP-020'],
    skipped: skipForgePhase5,
  });
  return {
    steps: phase5Steps.steps,
    stepsSkipped: phase5Steps.stepsSkipped,
    api: {
...
async function runGoldenPathPhases(options = {}) {
  ...
  assertRealEvidenceModeOptions(options);
  const outputPath = options.outputPath || DEFAULT_OUTPUT;
  let evidence = options.pilot || loadPilotEvidence(outputPath);
  if (!evidence) throw new Error(`Missing pilot evidence at ${outputPath}; run Phase 1 first.`); evidence = await collectRealEvidenceForRun(evidence, options);
  ...
  if (phaseOnly || (fromPhase <= 6 && toPhase >= 6 && fromPhase > 5)) {
    const phase6Result = await runGoldenPathPhase6(ctx, evidence = await collectRealEvidenceForRun(evidence, options), options);
    ...
    evidence.github = phase6GithubEvidence(evidence, phase6Result.api);
    evidence.deploy = {
      ...(evidence.deploy || {}),
  ...
  try {
    if (fromPhase <= 2 && toPhase >= 2) {
      ...
      evidence.stepsSkipped = mergeStepsSkipped(evidence.stepsSkipped, phase2Result.stepsSkipped);
      if (phase2Result.forgePolicy?.skipped) {
        Object.assign(evidence, applyForgeSkipToStepInventory(evidence, phase2Result.forgePolicy));
      } else if (phase2Result.forgePolicy) {
        evidence.forgePolicy = phase2Result.forgePolicy;
      }
      evidence.forgeadapter = {
        ...
        skipped: phase2Result.forgePolicy?.skipped === true,
        forgePolicy: phase2Result.forgePolicy || null,
      };
      evidence.github = {
        ...(evidence.github || {}),
        branchName: phase2Result.api.implementation.branchName, commitSha: phase2Result.api.implementation.commitSha,
        prUrl: phase2Result.api.implementation.prUrl, prNumber: phase2Result.api.implementation.prNumber,
      };
      evidence.phase2 = {
        completedAt: new Date().toISOString(),
        ...
        forgePolicy: phase2Result.forgePolicy || null,
        stepsSkipped: phase2Result.stepsSkipped || [],
      };
      savePilotEvidence(evidence, outputPath);
    }
    if (fromPhase <= 3 && toPhase >= 3) {
      ...
      evidence.stepsSkipped = mergeStepsSkipped(evidence.stepsSkipped, phase3Result.stepsSkipped);
      if (phase2Result.forgePolicy?.skipped) {
        Object.assign(evidence, applyForgeSkipToStepInventory(evidence, phase2Result.forgePolicy));
      }
      evidence.phase3 = {
        ...
        stepsSkipped: phase3Result.stepsSkipped || [],
      };
      savePilotEvidence(evidence, outputPath);
    }
    if (fromPhase <= 4 && toPhase >= 4) {
      ...
      evidence.stepsSkipped = mergeStepsSkipped(evidence.stepsSkipped, phase4Result.stepsSkipped);
      if (phase2Result.forgePolicy?.skipped) {
        Object.assign(evidence, applyForgeSkipToStepInventory(evidence, phase2Result.forgePolicy));
      }
      evidence.github = {
        ...(evidence.github || {}),
        commitSha: phase4Result.api.fixCommitSha,
      };
      evidence.phase4 = {
        completedAt: new Date().toISOString(),
        personas: buildPhasePersonaSnapshot(4, evidence, phase4Result.api),
        api: phase4Result.api,
        stepsSkipped: phase4Result.stepsSkipped || [],
      };
      savePilotEvidence(evidence, outputPath);
    }

    if (fromPhase <= 5 && toPhase >= 5) evidence = await applyTrustedSimpleMerge({ evidence, options, phaseResults, outputPath, saveEvidence: savePilotEvidence });
    ...
    if (fromPhase <= 5 && toPhase >= 5) {
      ...
      evidence.stepsSkipped = mergeStepsSkipped(evidence.stepsSkipped, phase5Result.stepsSkipped);
      if (phase2Result.forgePolicy?.skipped) {
        Object.assign(evidence, applyForgeSkipToStepInventory(evidence, phase2Result.forgePolicy));
      }
      evidence.phase5 = {
        ...
        stepsSkipped: phase5Result.stepsSkipped || [],
      };
      evidence.forgeadapter = {
        ...(evidence.forgeadapter || {}),
        ...
        skipped: phase2Result.forgePolicy?.skipped === true,
        forgePolicy: phase2Result.forgePolicy || evidence.forgeadapter?.forgePolicy || null,
      };
      savePilotEvidence(evidence, outputPath);
    }
    ...
    if (typeof phase2Result?.stack?.close === 'function') {
...
function phase6GithubEvidence(evidence, api) {
  const github = evidence.github || {};
  return { ...github, repository: api.repository || github.repository || null, prUrl: api.prUrl, prNumber: api.prNumber, merged: api.autoMerge?.merged === true || github.merged === true, mergeCommitSha: api.mergeCommitSha, mergedAt: api.autoMerge?.mergedAt || github.mergedAt };
}

async function collectRealEvidenceForRun(evidence, options) { if (!shouldCollectGoldenPathRealEvidence(options)) return evidence; const prepared = await prepareGoldenPathRealEvidence({ evidence, options }); Object.assign(options, prepared.options); return prepared.evidence; }
function assertRealEvidenceModeOptions(options = {}) {
  if (!isRealEvidenceRequired(options)) return;
  const disallowed = [options.skipValidation === true && 'skip validation', options.allowSreWaiver === true && 'SRE waiver', options.requireHealthyDeployment === false && 'disabled deployment health validation', ...fixtureDelegationFailures(options)].filter(Boolean);
  if (disallowed.length) throw new Error(`Autonomous golden-path real evidence mode cannot use ${disallowed.join(' or ')}`);
}
...
module.exports = {
  DEFAULT_STACK_PERSIST_DIR, DEFAULT_PILOT_PR_NUMBER, DEFAULT_PILOT_PR_URL,
  isRealEvidenceRequired, assertRealImplementationEvidence, assertRealPhase6Evidence,
  resolveGoldenPathStackPersistDir, readmeHasGoldenPathMarker, addReadmeGoldenPathMarker,
  runGoldenPathPhase2, runGoldenPathPhase3, runGoldenPathPhase4, runGoldenPathPhase5,
  runGoldenPathPhase6, runGoldenPathPhases, withLocalPhases,
