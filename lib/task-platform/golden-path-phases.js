...
async function resolveForgeExecutionStack(forge, options = {}) {
  const forgeAdapterBaseUrl = String(
    options.forgeAdapterBaseUrl || process.env.FORGEADAPTER_BASE_URL || '',
  ).replace(/\/+$/, '');
  const faToken = options.forgeAdapterToken || DEFAULT_FORGE_ADAPTER_TOKEN;
  ...
  if (forgeAdapterBaseUrl) {
    return {
      ...
      faToken,
      managed: false,
      source: 'configured_forgeadapter',
      async close() {},
    };
  }
