...
async function runGoldenPathPhase6(ctx, evidence, options = {}) { assertRealEvidenceModeOptions(options);
  ...
  if (isRealEvidenceRequired(options)) assertRealPhase6AutoMerge(api.autoMerge);
...
async function runGoldenPathPhase5(ctx, phase2Result, evidence, options = {}) {
  ...
  if (etTaskId && taskAlreadyClosed) {
    ...
    // Local live/fixture proof (not requireRealEvidence) seeds a synthetic merged PR so GP-026
    // can start without a real forge PR target. Trusted real-evidence mode still requires hasPrTarget.
    const prSyncedAt = new Date().toISOString();
    const localPrNumber = hasPrTarget ? prNumber : 900000 + (Number(String(etTaskId).replace(/\D/g, '').slice(-5)) || 1);
    const localPrUrl = hasPrTarget
      ? prUrl
      : `https://github.com/${repository || 'wiinc1/engineering-team'}/pull/${localPrNumber}`;
    const localMergeSha = hasPrTarget
      ? mergeCommitSha
      : (mergeCommitSha && String(mergeCommitSha).length === 40
        ? mergeCommitSha
        : crypto.createHash('sha1').update(`local-sre-pr:${etTaskId}:${prSyncedAt}`).digest('hex'));
    const localRepo = repository || 'wiinc1/engineering-team';
    api.prSyncForSre = await apiSend(ctx, `/tasks/${encodeURIComponent(etTaskId)}/events`, 'POST', ['admin'], {
      eventType: 'task.github_pr_synced',
      actorType: 'agent',
      idempotencyKey: `golden-path:pr-merged:${etTaskId}:${String(localMergeSha).slice(0, 12)}`,
      payload: {
        pr_number: localPrNumber,
        pr_title: goldenPathPrTitle(isRealEvidenceRequired(options)),
        state: 'closed',
        pr_state: 'merged',
        pr_merged: true,
        pr_repository: localRepo,
        merge_commit_sha: localMergeSha,
        pr_url: localPrUrl,
        pr_updated_at: prSyncedAt,
        linked_prs: [{
