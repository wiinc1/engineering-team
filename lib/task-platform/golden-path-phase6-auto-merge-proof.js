...
function assertRealPhase6AutoMerge(autoMerge = {}) {
  ...
  const failures = [];
  const confirmedAlreadyMerged = autoMerge.skipped === true
    && autoMerge.reason === 'already_merged'
    && autoMerge.merged === true;
  if (autoMerge.skipped === true && !confirmedAlreadyMerged) {
    failures.push(`auto-merge cannot be skipped${autoMerge.reason ? `: ${autoMerge.reason}` : ''}`);
  }
