...
function loadFactoryEvidence(observabilityDir) {
  return listJsonFiles(observabilityDir)
    .filter((p) => /factory-(?:milestone-c-|cohort-)/.test(path.basename(p))
      && !/real-delivery-candidate/.test(p))
    .map((filePath) => {
      const doc = readJson(filePath);
...
function buildSimpleTrustedCohortFromRepo(root = process.cwd(), options = {}) {
  ...
  const selectedTaskIds = new Set((options.taskIds || []).map((taskId) => String(taskId).trim()).filter(Boolean));
  const selected = (row) => selectedTaskIds.size === 0 || selectedTaskIds.has(row.taskId);
  const closeouts = loadCloseouts(closeoutDir, { root }).filter(selected);
  const factoryEvidence = loadFactoryEvidence(observabilityDir).filter(selected);
  const cohort = buildSimpleTrustedCohort({ closeouts, factoryEvidence, bar: options.bar || DEFAULT_BAR });
  cohort.selection = {
    cohortId: options.cohortId || null,
    mode: selectedTaskIds.size ? 'explicit_task_ids' : 'all_discovered',
    taskIds: [...selectedTaskIds].sort(),
  };
  return cohort;
}
