...
// GP-023 runs after the factory has loaded live service, queue, auth, and
// trusted-delivery configuration into its process. The validation subprocess
// must behave like a clean checkout test run, not inherit those runtime modes.
// Keep a small operating-system allowlist rather than trying to enumerate every
// product flag or credential that must be removed.
const VALIDATION_ENV_KEYS_TO_KEEP = Object.freeze([
  'PATH',
  'HOME',
  'USER',
  'LOGNAME',
...
function validationSubprocessEnv(env = process.env) {
  const next = {};
  for (const key of VALIDATION_ENV_KEYS_TO_KEEP) {
    if (env[key] != null && env[key] !== '') next[key] = env[key];
  }
