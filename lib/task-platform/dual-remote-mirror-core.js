 ...
 * Optional content flags resolve "both sides unique commits" when one tip's
 * tree is already contained in the other side's history (forge-local merge SHAs).
 *
 * @param {{
 *  divergence?: object,
 *  content?: {
 *    githubTreeInOriginHistory?: boolean,
 *    originTreeInGithubHistory?: boolean,
 *    githubIsAncestorOfOrigin?: boolean,
 *    originIsAncestorOfGithub?: boolean,
 *  },
 * }} report
 */
function decideMirrorAction(report = {}) {
  ...
  // Pure backup-behind (no unique GitHub commits)
  ...
  // Both sides have unique commits / trees differ: prefer content containment
  if (d.primaryBehindBackup && d.backupBehindPrimary) {
    ...
    if (githubContained && !originContained) {
      return {
        action: 'mirror_backup',
        exitCode: EXIT.BACKUP_BEHIND,
        reason: 'GitHub tip tree is already in GitLab history — mirror primary → backup.',
      };
    }
    if (originContained && !githubContained) {
      return {
        action: 'fail_primary_behind',
        exitCode: EXIT.PRIMARY_BEHIND,
        reason: 'GitLab tip tree is already in GitHub history — equalize primary before mirroring.',
      };
    }
    return {
