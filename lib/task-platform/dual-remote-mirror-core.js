...
function actionResult(action, exitCode, reason) {
  return { action, exitCode, reason };
}

function decideBothSidesUnique(content = {}) {
  const githubContained = content.githubTreeInOriginHistory === true
    || content.githubIsAncestorOfOrigin === true;
  const originContained = content.originTreeInGithubHistory === true
    || content.originIsAncestorOfGithub === true;
  if (githubContained && !originContained) {
    return actionResult(
      'mirror_backup',
      EXIT.BACKUP_BEHIND,
      'GitHub tip tree is already in GitLab history — mirror primary → backup.',
    );
  }
  if (originContained && !githubContained) {
    return actionResult(
      'fail_primary_behind',
      EXIT.PRIMARY_BEHIND,
      'GitLab tip tree is already in GitHub history — equalize primary before mirroring.',
    );
  }
  return actionResult(
    'fail_diverged',
    EXIT.ERROR,
    'Both sides have unique content — equalize into GitLab primary, then re-run mirror.',
  );
}

/**
 * Decide mirror agent action from a dual-remote status report.
 * @param {{ divergence?: object, content?: object, ancestry?: object }} report
 */
function decideMirrorAction(report = {}) {
  const d = report.divergence || {};
  const content = report.content || report.ancestry || {};
  if (d.synced) {
    return actionResult(
      'noop_synced',
      EXIT.SYNCED,
  ...
  if (d.backupBehindPrimary && !d.primaryBehindBackup) {
    return actionResult(
      'mirror_backup',
      EXIT.BACKUP_BEHIND,
      'GitHub backup is behind GitLab primary — push mirror branch and open/update PR.',
    );
  }
  if (d.primaryBehindBackup && !d.backupBehindPrimary) {
    return actionResult(
      'fail_primary_behind',
      EXIT.PRIMARY_BEHIND,
      'GitLab primary is behind GitHub content — equalize into GitLab first (do not force-overwrite).',
    );
  }
  if (d.primaryBehindBackup && d.backupBehindPrimary) {
    return decideBothSidesUnique(content);
  }
  ...
  return actionResult(
    'fail_unknown',
    EXIT.ERROR,
    'Unable to classify dual-remote divergence; inspect remotes:sync-status.',
  );
}
