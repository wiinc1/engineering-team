const { attachFactoryApprovalProvenance, recordGoldenPathHumanReview } = require('./golden-path-human-review');
...
async function approveExecutionContractWithRetry(ctx, helpers, taskId) {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    if (lastResponse.ok) {
      lastResponse.projectionPostApproval = await runProjectionCatchUp(ctx, 'execution-contract-approved');
