...
function buildIntakeDescription(requirement) {
  return [
    ...
    requirement.changeKind ? `Change kind: ${requirement.changeKind}` : '',
    requirement.changedFiles?.length ? `Changed files: ${requirement.changedFiles.join(', ')}` : '',
  ].filter(Boolean).join('\n');
}
...
async function createTask(ctx, requirement, projectName) {
  const createTask = await apiSend(ctx, '/api/v1/tasks', 'POST', ['admin'], {
    ...
    metadata: {
      ...
      change_kind: requirement.changeKind || null,
      changed_files: requirement.changedFiles || [],
