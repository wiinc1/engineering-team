...
const DEFAULT_BASE_URL = 'https://engineering-team-zeta.vercel.app';
...
function buildTaskDescription({ childIssueUrl, epicIssueNumber, childIssueNumber }) {
  return [
    ...
    '- Vercel production deploy recorded',
    '',
...
async function runGoldenPathPhase0(options = {}) {
  ...
  const assignedTask = data(api.assignPm);

  api.attachTask = await apiSend(ctx, `/api/v1/tasks/${encodeURIComponent(taskId)}/project`, 'PATCH', ['pm'], {
    projectId,
