...
/**
 * Operator-trusted delivery (real PR/commit evidence). Distinct from live session proof:
 * live OpenClaw sessions may still use session-proof mode without real PR artifacts.
 */
function isTrustedDeliveryMode(options = {}, env = process.env) {
  if (options.sessionProofOnly === true || options.allowSyntheticEvidence === true) return false;
  if (options.requireRealEvidence === true || options.trustedDelivery === true) return true;
  if (options.collectRealEvidence === true) return true;
  if (options.requireTrustedSimpleClose === true || options.trustedSimpleClose === true) return true;
  const truthy = (value) => ['1', 'true', 'yes', 'on'].includes(String(value || '').trim().toLowerCase());
  return truthy(env.FF_GOLDEN_PATH_REQUIRE_REAL_EVIDENCE)
    || truthy(env.FACTORY_TRUSTED_DELIVERY)
    || truthy(env.FF_FACTORY_TRUSTED_SIMPLE_CLOSE);
}
...
function buildImplementerPrompt({
  ...
  const trusted = isTrustedDeliveryMode({
    requireRealEvidence,
    trustedDelivery,
    trustedSimpleClose,
    sessionProofOnly,
  ...
  const executionRules = trusted && runKind === 'fix_after_qa_fail' ? [
    ...
    'Address the QA finding, run focused tests, commit, and push to that same branch and pull request. Do not open a second pull request.',
    'Wait for every pre-merge check already attached to the updated pull request, except the factory-owned Merge readiness check, to pass. Do not merge; the factory owns the merge.',
    ...
    'Do not use tools or edit files; this session is attribution proof only.',
  ];
  return [
    ...
    evidenceRule,
    ...executionRules,
    'After all work completes, reply with JSON only and no prose.',
    'Required JSON keys: branchName (string), commitSha (40-char hex), prUrl (https PR URL).',
    'Requirements summary:',
    String(requirements || '(none provided)').slice(0, 1600),
  ].join('\n');
}

function buildQaPrompt({
  taskId, requirements, runKind = 'initial', priorRunId = null,
  repository, branchName, commitSha, prUrl, changedFiles = [],
  requireRealEvidence = false, trustedDelivery = false, trustedSimpleClose = false,
  sessionProofOnly = false, allowSyntheticEvidence = false, collectRealEvidence = false,
  proofProfile,
} = {}) {
  const trusted = isTrustedDeliveryMode({
    requireRealEvidence,
  ...
  const reviewRules = trusted ? [
    'TRUSTED DELIVERY QA: use read-only filesystem, git, and GitHub commands to inspect the supplied exact pull request and commit.',
    'Confirm the diff matches the approved requirements and expected changed files, and confirm the attached pre-merge checks are successful.',
    'Do not edit files or pull-request metadata, push commits, rerun checks, emit Merge readiness, or merge the pull request.',
    `Repository: ${repository || '(not provided)'}`,
    `Branch: ${branchName || '(not provided)'}`,
    `Commit SHA: ${commitSha || '(not provided)'}`,
    `Pull request: ${prUrl || '(not provided)'}`,
    `Expected changed files: ${Array.isArray(changedFiles) && changedFiles.length ? changedFiles.join(', ') : '(not provided)'}`,
  ] : [
  ...
  return [
    ...
    ...reviewRules,
    'After review, reply with JSON only and no prose.',
...
function resolveImplementerArtifacts(delegation = {}, options = {}) {
  ...
  const trusted = isTrustedDeliveryMode(options);
  if (trusted || options.requireRealEvidence === true) {
  ...
  // Never invent commitSha under trusted delivery; session-proof may generate local placeholders.
  const commitSha = providedCommitSha
    || (trusted ? null : crypto.randomBytes(20).toString('hex'));
  ...
  return {
    ...
    trustedDelivery: trusted,
    sessionProofOnly: !trusted,
  };
}
...
async function runImplementerAgentPhase(ctx, options = {}) {
  ...
  const delegation = await delegateFactorySpecialist(
    ...
    buildImplementerPrompt(options),
    {
      ...
      delegationRunnerTimeoutMs: trusted ? 31 * 60 * 1000 : undefined,
      openclawTimeoutSec: trusted ? 30 * 60 : undefined,
    },
  );
