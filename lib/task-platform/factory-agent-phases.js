...
function buildImplementerPrompt({ taskId, requirements, engineerTier = 'Jr', runKind = 'initial' } = {}) {
  return [
    `You are the ${engineerTier} engineer agent implementing task ${taskId}.`,
    `Run kind: ${runKind}.`,
    'Implement the docs-only factory delivery marker and return JSON with commitSha and prUrl.',
    'Requirements:',
...
function buildQaPrompt({ taskId, requirements, runKind = 'initial', priorRunId = null } = {}) {
  return [
    `You are the QA agent verifying task ${taskId}.`,
    `Run kind: ${runKind}.`,
    priorRunId ? `Prior run id: ${priorRunId}` : '',
    'Return JSON with outcome pass|fail and findings array.',
    'Requirements:',
    requirements || '(none provided)',
  ].join('\n');
}
...
function buildSrePrompt({ taskId, operatorUrl, mergeCommitSha, requirements } = {}) {
  return [
    `You are the SRE agent approving the monitoring window for task ${taskId}.`,
    `Operator URL: ${operatorUrl || '(not provided)'}`,
    mergeCommitSha ? `Deployment version: ${mergeCommitSha}` : '',
    'Return JSON with approved (boolean), reason (string), and evidence (string array).',
    'Requirements:',
    requirements || '(none provided)',
  ].filter(Boolean).join('\n');
}
...
function resolveImplementerArtifacts(delegation = {}, options = {}) {
  ...
  return { commitSha, prUrl, parsed, delegated: delegation.delegated === true };
}
...
function buildCiValidationEvidence(validation = {}, options = {}) {
  ...
  return {
    ...
    repository,
    ciUrl: options.ciUrl || `https://github.com/${repository}/actions/workflows/${path.basename(workflow)}`,
    local: validation,
