...
async function resolveFactoryProofProfile(options = {}) {
  ...
  // Hermes default is de-scoped (#272). When forced required, fail closed on unset/mock.
  const hermesClaim = assertHermesEligibleForClaim({
    hermesUrl: options.hermesUrl || env.HERMES_BASE_URL || '',
    env,
    proofProfile: explicit || 'live',
  });
