...
function buildPhaseRunnerOptions(config, item) {
  ...
  // Factory phases always call the HTTP API; persistDir null keeps postgres projection catch-up active.
...
async function runFactoryExecutionPhases(config, item) {
  ...
  await runGoldenPathPhases({ ...options, ...phaseRange, pilot: evidence });
  const refreshed = loadPilotEvidence(options.outputPath);
  ...
  return {
    ...
    completedAt: phase6Complete ? new Date().toISOString() : null,
  };
}
