...
const { assertForgeSkipAllowed } = require('./forge-claim-policy');
const { requestFactoryForgeArchitectAssignment } = require('./factory-forge-architect-assignment');

function assertOrSeedFactoryForge(config, item, forgeTaskId, evidence = null) {
  if (config.skipForgeSeed) {
    assertForgeSkipAllowed({
      skipForgeSeed: true,
      allowForgeSkip: config.allowForgeSkip === true,
  ...
  return seedFactoryForgeTask(config, forgeTaskId);
}
...
async function runFactoryPhase1(config, item) {
  ...
  await assertOrSeedFactoryForge(config, item, options.forgeTaskId, refreshed);
  return {
    ...item,
...
async function runFactoryExecutionPhases(config, item) {
  ...
  assertFactoryItemRealEvidencePreflight(config, item, evidence);
  await verifyFactoryRealDeliveryCandidate(config, item);
  if (!phaseRange.resumePhase6Only) {
    await assertOrSeedFactoryForge(config, item, options.forgeTaskId, evidence);
  }
  applyForgeLifecycleEnv(options.forgeTaskId);
  Object.assign(process.env, factoryDelegationEnv(config, item));
  await runPhasesFn({
    ...options,
    ...phaseRange,
    templateTier: item.templateTier || evidence?.engineeringTeam?.templateTier || options.templateTier,
    pilot: evidence,
...
module.exports = {
  ...
  normalizeRequirement, submitFactoryRequirements, submitFactoryRequirementsForQueue, requestFactoryForgeArchitectAssignment,
