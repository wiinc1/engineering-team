...
function deriveCloseGovernanceProjection({
  ...
  const latestDecision = humanDecision || architectCancellationRecommendation || pmCancellationRecommendation || backtrackDecision || null;
  const escalationProjection = latestEscalation ? {
    ...
    awaitingHumanDecision: summary?.waiting_state === 'awaiting_human_stakeholder_escalation',
  } : null;
  ...
  const humanDecisionRequired = summary?.current_stage === STAGES.PM_CLOSE_REVIEW
    && (summary?.waiting_state === 'awaiting_human_close_review'
      || summary?.waiting_state === 'awaiting_human_stakeholder_escalation'
      || (pmCancellationRecommendation && architectCancellationRecommendation && humanDecision?.outcome !== 'approve' && humanDecision?.outcome !== 'reject')
  ...
  return {
    ...
    cancellation: {
      ...
      awaitingHumanDecision: Boolean(pmCancellationRecommendation && architectCancellationRecommendation)
    ...
    humanDecision: {
      ...
      summary: humanDecision?.summary
        || (pmCancellationRecommendation && architectCancellationRecommendation
...
function createAuditApiServer(options = {}) {
  ...
  const runtimeOptions = { ...options, jwtVerifier };
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (routePath === '/tasks' && req.method === 'POST') {
        ...
        taskPlatform.createTask({
          tenantId: context.tenantId,
          actorId: context.actorId,
          taskId,
          title: body.title,
          description: body.business_context,
          status: 'DRAFT',
          priority: body.priority,
          idempotencyKey: `task-platform:create:${taskId}`,
          requestId,
        });
      ...
      if (!summaryMatch && !resourceMatch && !assignmentMatch && !lockMatch && !skillEscalationMatch && !checkInsMatch && !retierMatch && !reassignmentMatch && !sreMonitoringMatch && !pmBusinessContextMatch && !closeReviewCancellationMatch && !closeReviewExceptionalDisputeMatch && !closeReviewHumanDecisionMatch && !closeReviewBacktrackMatch && !architectHandoffMatch && !engineerSubmissionMatch && !workflowThreadsMatch && !workflowThreadActionMatch && !qaResultsMatch && !reviewQuestionsMatch && !reviewQuestionActionMatch) throw createHttpError(404, 'not_found', 'Route not found');
