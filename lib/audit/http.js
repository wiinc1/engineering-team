...
function createAuditApiServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      const executionContractMatch = routePath.match(/^\/tasks\/([^/]+)\/execution-contract(?:\/(validate|markdown|approve))?$/);
      ...
      const resource = assignmentMatch
        ...
        : executionContractMatch
          ? `execution-contract${executionContractMatch[2] ? `:${executionContractMatch[2]}` : ''}`
        : orchestrationMatch
          ? 'orchestration'
