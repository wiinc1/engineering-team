...
function createAuditApiServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (!summaryMatch && !resourceMatch && !assignmentMatch && !architectHandoffMatch && !engineerSubmissionMatch && !reviewQuestionsMatch && !reviewQuestionActionMatch) throw createHttpError(404, 'not_found', 'Route not found');
      const taskId = (summaryMatch || resourceMatch || assignmentMatch || architectHandoffMatch || engineerSubmissionMatch || reviewQuestionsMatch || reviewQuestionActionMatch)[1];
