...
} = require('./deferred-considerations');
...
const { deriveControlPlaneProjection } = require('./control-plane');
...
const validateMergeReadinessReviewCreateRequest = (body) => validateRequest(body, {
  required: ['repository', 'pullRequestNumber', 'commitSha', 'reviewStatus'],
  properties: {
    ...
    reviewerActorId: { type: 'string' },
    reviewerActorType: { type: 'string' },
  },
});

const validateMergeReadinessReviewUpdateRequest = (body) => validateRequest(body, {
  required: ['reviewId', 'recordVersion'],
  properties: {
...
function executionContractAutoApprovalRequested(body = {}) {
  const value = body.autoApproval ?? body.auto_approval ?? body.policyAutoApproval ?? body.policy_auto_approval;
  if (typeof value === 'boolean') return value;
  if (typeof value === 'string') return normalizeBoolean(value, false);
  if (value && typeof value === 'object' && !Array.isArray(value)) {
    return normalizeBoolean(value.requested ?? value.enabled ?? value.approvedByPolicy ?? value.approved_by_policy, true);
  }
  return false;
}

async function recordExecutionContractAutoApprovalMetric(store, updates = {}) {
  await updateStoreMetrics(store, (metrics) => {
    metrics.feature_execution_contract_auto_approvals_total = Number(metrics.feature_execution_contract_auto_approvals_total || 0) + Number(updates.approved || 0);
    metrics.feature_execution_contract_auto_approval_blocked_total = Number(metrics.feature_execution_contract_auto_approval_blocked_total || 0) + Number(updates.blocked || 0);
    metrics.feature_operator_trusted_autonomous_deliveries_total = Number(metrics.feature_operator_trusted_autonomous_deliveries_total || 0) + Number(updates.trustedDelivery || 0);
    const approved = Number(metrics.feature_execution_contract_auto_approvals_total || 0);
    const deliveries = Number(metrics.feature_operator_trusted_autonomous_deliveries_total || 0);
    metrics.feature_operator_trusted_autonomous_delivery_rate = approved > 0 ? deliveries / approved : 0;
  });
}

async function recordContractCoverageAuditMetric(store, updates = {}) {
  await updateStoreMetrics(store, (metrics) => {
    metrics.feature_contract_coverage_audits_submitted_total = Number(metrics.feature_contract_coverage_audits_submitted_total || 0) + Number(updates.submitted || 0);
    metrics.feature_contract_coverage_audits_closed_total = Number(metrics.feature_contract_coverage_audits_closed_total || 0) + Number(updates.closed || 0);
    metrics.feature_contract_coverage_implementation_incomplete_total = Number(metrics.feature_contract_coverage_implementation_incomplete_total || 0) + Number(updates.implementationIncomplete || 0);
    metrics.feature_autonomy_confidence_positive_signals_total = Number(metrics.feature_autonomy_confidence_positive_signals_total || 0) + Number(updates.positive || 0);
    metrics.feature_autonomy_confidence_neutral_signals_total = Number(metrics.feature_autonomy_confidence_neutral_signals_total || 0) + Number(updates.neutral || 0);
    metrics.feature_autonomy_confidence_negative_signals_total = Number(metrics.feature_autonomy_confidence_negative_signals_total || 0) + Number(updates.negative || 0);
    metrics.feature_autonomy_confidence_signal_score = Number(metrics.feature_autonomy_confidence_signal_score || 0)
      + Number(updates.positive || 0)
      - Number(updates.negative || 0);
  });
}

async function maybeRecordOperatorTrustedAutonomousDelivery({ store, taskId, tenantId, closeEvent = null }) {
  if (typeof store.getTaskHistory !== 'function') return;
  const history = await store.getTaskHistory(taskId, { tenantId });
  const closeEventIds = new Set(history
    .filter((event) => event?.event_type === 'task.closed')
    .map((event) => event.event_id || String(event.sequence_number || '')));
  if (closeEvent?.event_type === 'task.closed') {
    closeEventIds.add(closeEvent.event_id || String(closeEvent.sequence_number || 'current-close'));
  }
  if (closeEventIds.size !== 1) return;
  const executionContract = deriveExecutionContractProjection(history);
  const autoApproval = executionContract.approval?.autoApproval || executionContract.latest?.auto_approval || null;
  if (autoApproval?.approved_by_policy) {
    await recordExecutionContractAutoApprovalMetric(store, { trustedDelivery: 1 });
  }
}
...
function evaluateApprovedContractDispatchForAssignment(history = [], { proposedAssignee = null, proposedEngineerTier = null } = {}) {
  const projection = deriveExecutionContractProjection(history);
  if (!projection.active || !projection.approval) return null;
  return evaluateExecutionContractDispatchReadiness({
    contract: projection.latest,
    verificationReport: projection.verificationReport,
    proposedEngineerTier,
    proposedAssignee,
  });
}

function assertApprovedContractDispatchAllowed(readiness, errorFactory = createHttpError) {
  if (!readiness || readiness.canDispatch) return;
  const primary = readiness.blockedReasons?.[0];
  throw errorFactory(
    409,
    'dispatch_policy_blocked',
    primary?.detail || 'Execution Contract dispatch policy blocked this assignment.',
    {
      dispatch_readiness: readiness,
      blocking_reasons: readiness.blockedReasons || [],
      recommended_engineer_tier: readiness.dispatchPolicy?.selectedEngineerTier || null,
      recommended_assignee: readiness.dispatchPolicy?.selectedAssignee || null,
    },
  );
}
...
function buildQaEscalationPackage({ taskId, summary, architectHandoff, implementationHistory, latestFailedQa, executionContract, body, context }) {
  ...
  const dispatchPolicy = evaluateExecutionContractDispatchPolicy({
    contract: executionContract?.latest || {},
    verificationReport: executionContract?.verificationReport || null,
    implementingEngineerTier: requiredEngineerTier,
    implementingAssignee: recipientAgentId,
    failureContext: {
      priorFailedQaCount: latestFailedQa ? 1 : 0,
      highRiskRegression: body.high_risk_regression,
      unclearRootCause: body.unclear_root_cause,
      contractDefectSuspected: body.contract_defect_suspected,
      principalEscalationRequested: body.principal_escalation_requested,
    },
  });
  const escalationChain = dispatchPolicy.failureLoop.escalationChain;
  ...
  return {
    ...
    routing: {
      ...
      failure_loop_policy: dispatchPolicy.failureLoop,
    },
    ...
    failure_loop: dispatchPolicy.failureLoop,
    attachment_strategy: {
...
function validateQaResultBody(body = {}) {
  ...
  const normalized = {
    ...
    high_risk_regression: parseBooleanOption(body.highRiskRegression ?? body.high_risk_regression, false),
    unclear_root_cause: parseBooleanOption(body.unclearRootCause ?? body.unclear_root_cause, false),
    contract_defect_suspected: parseBooleanOption(body.contractDefectSuspected ?? body.contract_defect_suspected, false),
    principal_escalation_requested: parseBooleanOption(body.principalEscalationRequested ?? body.principal_escalation_requested, false),
  };
...
function canManageDeferredConsiderations(context) {
  return hasAnyRole(context, ['pm', 'admin']);
}

function canOperateDeferredConsiderations(context) {
  return hasAnyRole(context, ['pm', 'stakeholder', 'operator', 'admin']);
}
...
function assertNoRestrictedExecutionContractEventWrite(eventType) {
  ...
  if (eventType === 'task.contract_coverage_audit_submitted' || eventType === 'task.contract_coverage_audit_validated') {
    throw createHttpError(403, 'forbidden', 'Contract Coverage Audit events must use the dedicated coverage endpoint so implementation, QA, and closeout gates run.');
  }
  if (isDeferredConsiderationEventType(eventType)) {
    throw createHttpError(403, 'forbidden', 'Deferred Consideration records must use the dedicated endpoints so scope, promotion, and closeout rules run.');
  }
}
...
function deriveCloseGovernanceProjection({
  ...
  const deferredConsiderations = deriveDeferredConsiderations(history);
  ...
  return {
    ...
    deferredConsiderations: closeoutProjectionFromDeferredConsiderations(deferredConsiderations),
...
function summarizeTaskForDetail(taskId, state, history = []) {
  ...
  const executionContract = deriveExecutionContractProjection(history);
  const controlPlane = deriveControlPlaneProjection(history, state);
  const deferredConsiderations = deriveDeferredConsiderations(history);
...
function summarizeTaskForList(summary) {
  return {
    ...
    control_plane: summary.control_plane || { active: false, decisions: [], exceptions: [] },
    deferred_considerations: summary.deferred_considerations || { items: [], unresolved: [], summary: { total: 0, unresolved_count: 0 } },
...
function buildTaskDetailViewModel({
  ...
  return {
    ...
    context: {
      ...
      controlPlane: summary.control_plane || { active: false, decisions: [], exceptions: [] },
...
function canSubmitContractCoverageAudit(context) {
  return hasAnyRole(context, ['engineer', 'admin']);
}

function canValidateContractCoverageAudit(context) {
  return hasAnyRole(context, ['qa', 'admin']);
}
...
async function loadDeferredConsiderationContext(store, taskId, tenantId) {
  const [state, history] = await Promise.all([
    store.getTaskCurrentState(taskId, { tenantId }),
    store.getTaskHistory(taskId, { tenantId, limit: 500 }),
  ]);
  if (!state) throw createHttpError(404, 'task_not_found', 'Task not found', { task_id: taskId });
  const summary = summarizeTaskForDetail(taskId, state, history);
  const projection = deriveDeferredConsiderations(history);
  return { state, history, summary, projection };
}

function getDeferredConsiderationOrThrow(projection, considerationId) {
  const item = projection.items.find((candidate) => candidate.id === considerationId || candidate.deferred_consideration_id === considerationId);
  if (!item) {
    throw createHttpError(404, 'deferred_consideration_not_found', 'Deferred Consideration not found.', {
      deferred_consideration_id: considerationId,
    });
  }
  return item;
}

function assertDeferredConsiderationOpen(item) {
  if (!['captured', 'reviewed'].includes(item.status)) {
    throw createHttpError(409, 'deferred_consideration_already_resolved', 'Only unresolved Deferred Considerations can be reviewed, promoted, or closed.', {
      deferred_consideration_id: item.id,
      status: item.status,
    });
  }
}

async function listDeferredConsiderationReviewQueue({ store, tenantId }) {
  ...
  const tasks = await store.listTaskSummaries({ tenantId });
  const items = (await Promise.all(tasks.map(async (task) => {
    const history = typeof store.getTaskHistory === 'function'
      ? await store.getTaskHistory(task.task_id, { tenantId, limit: 500 })
      : [];
    return deriveDeferredConsiderations(history).unresolved.map((consideration) => ({
      ...consideration,
      source_task: {
  ...
  const buildGroup = (groupName, keyFn, labelFn = keyFn) => Object.values(items.reduce((acc, item) => {
    const rawKey = keyFn(item) || 'unspecified';
    const key = String(rawKey);
    if (!acc[key]) {
      acc[key] = {
        group: groupName,
        key,
        label: String(labelFn(item) || rawKey),
        count: 0,
        items: [],
      };
    }
  ...
  return {
    ...
    items,
    groups: {
      by_revisit_date: buildGroup('revisit_date', (item) => item.revisit_date || 'unscheduled', (item) => item.revisit_date ? `Revisit date: ${item.revisit_date}` : 'Unscheduled'),
      by_revisit_trigger: buildGroup('revisit_trigger', (item) => item.revisit_trigger || 'no_trigger', (item) => item.revisit_trigger ? `Trigger: ${item.revisit_trigger}` : 'No trigger'),
      by_source_task: buildGroup('source_task', (item) => item.source_task?.task_id || item.source_task_id, (item) => `${item.source_task?.task_id || item.source_task_id}: ${item.source_task?.title || item.title}`),
    },
    summary: {
      total: items.length,
      by_status: {
      ...
      by_owner: items.reduce((acc, item) => {
        const owner = item.owner || 'unassigned';
        acc[owner] = Number(acc[owner] || 0) + 1;
        return acc;
      }, {}),
    },
  };
...
async function captureDeferredConsideration({ store, taskId, tenantId, context, body = {}, source = 'http' }) {
  const { history, projection } = await loadDeferredConsiderationContext(store, taskId, tenantId);
  const requestedId = String(body.id || body.deferred_consideration_id || body.deferredConsiderationId || '').trim();
  const considerationId = requestedId || `DC-${taskId}-${String(projection.summary.total + 1).padStart(3, '0')}`;
  if (projection.items.some((item) => item.id === considerationId)) {
    throw createHttpError(409, 'deferred_consideration_id_exists', 'Deferred Consideration ID already exists for this task.', {
      deferred_consideration_id: considerationId,
    });
  }
  const occurredAt = new Date().toISOString();
  const deferredConsideration = normalizeDeferredConsiderationInput(body, {
    taskId,
    history,
    considerationId,
    actorId: context.actorId,
  ...
  const result = await store.appendEvent({
    ...
    actorId: context.actorId,
    actorType: body.actorType || 'user',
    idempotencyKey: body.idempotencyKey || `deferred-consideration:capture:${taskId}:${considerationId}`,
    occurredAt,
    payload: {
      deferred_consideration_id: considerationId,
  ...
  return { result, deferredConsideration };
}

async function reviewDeferredConsideration({ store, taskId, tenantId, context, considerationId, body = {}, source = 'http' }) {
  const { projection } = await loadDeferredConsiderationContext(store, taskId, tenantId);
  const consideration = getDeferredConsiderationOrThrow(projection, considerationId);
  assertDeferredConsiderationOpen(consideration);
  const review = normalizeDeferredConsiderationReview(body);
  const result = await store.appendEvent({
    taskId,
    tenantId,
    eventType: 'task.deferred_consideration_reviewed',
    actorId: context.actorId,
    actorType: body.actorType || 'user',
    idempotencyKey: body.idempotencyKey || `deferred-consideration:review:${taskId}:${consideration.id}:${projection.summary.total}:${review.action}`,
    payload: {
      deferred_consideration_id: consideration.id,
      ...review,
    },
    source,
  });
  ...
  if (review.action === 'convert_blocker') {
    if (review.conversion_type === 'refinement_blocking_question') {
      conversionEvent = await store.appendEvent({
        taskId,
        tenantId,
        eventType: 'task.workflow_thread_created',
        actorId: context.actorId,
        actorType: body.actorType || 'user',
        idempotencyKey: `deferred-consideration:blocking-question:${taskId}:${consideration.id}:${result.event.event_id}`,
        occurredAt: result.event.occurred_at,
        payload: {
          thread_id: `wf-dc-${consideration.id.replace(/[^a-z0-9-]/gi, '').slice(-12) || crypto.randomUUID().slice(0, 8)}`,
          comment_type: 'question',
          title: `Deferred Consideration blocks current scope: ${consideration.title}`,
          body: review.review_note || consideration.known_context || consideration.rationale,
          blocking: true,
          blocked: true,
          linked_event_id: result.event.event_id,
          deferred_consideration_id: consideration.id,
          notification_targets: ['pm', 'architect'],
          waiting_state: 'workflow_thread_resolution',
          next_required_action: 'Resolve the converted Deferred Consideration blocker before current work continues.',
        },
        source,
      });
    } else if (review.conversion_type === 'operator_decision_required_exception') {
      conversionEvent = await store.appendEvent({
        taskId,
        tenantId,
        ...
        actorId: context.actorId,
        actorType: body.actorType || 'user',
        idempotencyKey: `deferred-consideration:operator-exception:${taskId}:${consideration.id}:${result.event.event_id}`,
        occurredAt: result.event.occurred_at,
        payload: {
          reason: 'operator_decision_required',
          severity: 'warning',
          summary: `Deferred Consideration requires operator decision: ${consideration.title}`,
          rationale: review.review_note || consideration.rationale,
          deferred_consideration_id: consideration.id,
          waiting_state: 'operator_decision_required',
          next_required_action: 'Operator decision required before current work continues.',
        },
        source,
      });
      await store.appendEvent({
        ...
        actorId: context.actorId,
        actorType: body.actorType || 'user',
        idempotencyKey: `deferred-consideration:block:${taskId}:${consideration.id}:${result.event.event_id}`,
        occurredAt: result.event.occurred_at,
        payload: {
          blocker_type: 'operator_decision_required',
          reason: `Deferred Consideration ${consideration.id} was converted because it blocks current progress.`,
          summary: consideration.title,
          waiting_state: 'operator_decision_required',
          next_required_action: 'Operator decision required before current work continues.',
          freeze_scope: ['stage_transitions'],
          deferred_consideration_id: consideration.id,
        },
        source,
      });
  ...
  return { result, consideration, review, conversionEvent };
}

async function closeDeferredConsideration({ store, taskId, tenantId, context, considerationId, body = {}, source = 'http' }) {
  const { projection } = await loadDeferredConsiderationContext(store, taskId, tenantId);
  const consideration = getDeferredConsiderationOrThrow(projection, considerationId);
  assertDeferredConsiderationOpen(consideration);
  const closePayload = normalizeDeferredConsiderationClose(body);
  const result = await store.appendEvent({
    taskId,
    tenantId,
    eventType: 'task.deferred_consideration_closed',
    actorId: context.actorId,
    actorType: body.actorType || 'user',
    idempotencyKey: body.idempotencyKey || `deferred-consideration:close:${taskId}:${consideration.id}`,
    payload: {
      deferred_consideration_id: consideration.id,
      ...closePayload,
    },
  ...
  return { result, consideration, closePayload };
}

async function promoteDeferredConsideration({
  store,
  taskPlatform,
  taskId,
  tenantId,
  ...
  const { summary, projection } = await loadDeferredConsiderationContext(store, taskId, tenantId);
  const consideration = getDeferredConsiderationOrThrow(projection, considerationId);
  assertDeferredConsiderationOpen(consideration);
  const promoteBody = normalizePromoteBody(body);
  const promotedTaskId = `TSK-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
  const rawRequirements = buildPromotedIntakeRawRequirements(consideration, summary, promoteBody);
  const { created, createdTaskId, refinementRequested } = await persistTaskCreation({
    ...
    body: {
      mode: 'intake',
      title: promoteBody.title || consideration.title,
      rawRequirements,
      priority: promoteBody.priority || summary.priority || 'P3',
      taskType: promoteBody.task_type,
      idempotencyKey: promoteBody.idempotency_key || `deferred-consideration:promote-intake:${taskId}:${consideration.id}`,
      actorType: promoteBody.actor_type,
    },
  ...
  const promotionLink = {
    ...
    route: `/tasks/${encodeURIComponent(createdTaskId)}`,
  };
  const result = await store.appendEvent({
    taskId,
    tenantId,
    eventType: 'task.deferred_consideration_promoted',
    actorId: context.actorId,
    actorType: promoteBody.actor_type,
    idempotencyKey: `deferred-consideration:promoted:${taskId}:${consideration.id}:${createdTaskId}`,
    payload: {
      deferred_consideration_id: consideration.id,
      promoted_intake_task_id: createdTaskId,
      promotion_link: promotionLink,
      raw_requirements: rawRequirements,
      promotion_note: promoteBody.promotion_note,
      policy_version: DEFERRED_CONSIDERATION_POLICY_VERSION,
    },
    source,
  });
  return { result, consideration, promotedTaskId: createdTaskId, promotionLink, created, refinementRequested, rawRequirements };
}
...
async function approveLatestExecutionContract({ store, taskId, tenantId, context, body = {}, source = 'http' }) {
  ...
  const deferredConsiderations = deriveDeferredConsiderations(history);
  approvalSummary.deferredConsiderationsNotInScope = deferredConsiderations.unresolved.map(compactDeferredConsiderationForApproval);
  approvalSummary.deferredConsiderationsExcludedFromCoverage = true;
  approvalSummary.deferredConsiderationsPolicyVersion = DEFERRED_CONSIDERATION_POLICY_VERSION;
  const autoApprovalRequested = executionContractAutoApprovalRequested(body);
  const autoApprovalPolicy = evaluateExecutionContractAutoApprovalPolicy({
    contract: projection.latest,
    approvalSummary,
  });
  if (autoApprovalRequested && !autoApprovalPolicy.canAutoApprove) {
    await recordExecutionContractAutoApprovalMetric(store, { blocked: 1 });
    throw createHttpError(409, 'execution_contract_auto_approval_blocked', 'Auto-approval is blocked; explicit Operator Approval is required.', {
      auto_approval_policy: autoApprovalPolicy,
      policy_version: AUTO_APPROVAL_POLICY_VERSION,
      blocking_reasons: autoApprovalPolicy.blockingReasons,
  ...
  const autoApproval = autoApprovalRequested
    ? buildExecutionContractAutoApprovalRecord({
  ...
  const result = await store.appendEvent({
    ...
    actorId: context.actorId,
    actorType: body.actorType || (autoApproval ? 'policy' : 'user'),
    ...
    payload: {
      ...
      validation,
      approval_summary: approvalSummary,
      approval_mode: autoApproval ? 'policy' : 'explicit',
      auto_approval: autoApproval,
      auto_approval_policy: autoApprovalPolicy,
      dispatch_policy: dispatchPolicy,
  ...
  if (autoApproval && !result.duplicate) {
    await recordExecutionContractAutoApprovalMetric(store, { approved: 1 });
  }
  return { result, validation, contract: projection.latest, committedScope, approvalSummary, dispatchPolicy, autoApproval, autoApprovalPolicy };
}
...
function assertApprovedContractForCoverage(projection, taskId) {
  if (!projection.latest) {
    throw createHttpError(404, 'execution_contract_not_found', 'No Execution Contract version exists for this task.', { task_id: taskId });
  }
  if (!projection.approval || Number(projection.approval.version) !== Number(projection.latest.version)) {
    throw createHttpError(409, 'execution_contract_not_approved', 'Contract Coverage Audit requires the latest approved Execution Contract.', {
      latest_version: projection.latest.version,
    });
  }
}

async function submitContractCoverageAudit({ store, taskId, tenantId, context, body = {}, source = 'http' }) {
  const { state, history, projection } = await loadExecutionContractContext(store, taskId, tenantId);
  assertApprovedContractForCoverage(projection, taskId);
  if (![STAGES.IMPLEMENTATION, STAGES.IN_PROGRESS].includes(state.current_stage)) {
    throw createHttpError(409, 'invalid_stage', 'Contract Coverage Audit can only be submitted during Implementation.', {
      current_stage: state.current_stage,
    });
  }
  const implementationHistory = deriveImplementationHistory(history);
  const latestImplementation = implementationHistory[0] || null;
  if (!latestImplementation) {
    throw createHttpError(409, 'missing_engineer_submission', 'Contract Coverage Audit requires implementation metadata for the current attempt.', {
      current_stage: state.current_stage,
    });
  }
  const previousAudit = deriveContractCoverageAuditProjection(history, projection.latest);
  const submittedAt = new Date().toISOString();
  const { audit, readiness } = createContractCoverageAudit({
    taskId,
    contract: projection.latest,
    implementationAttempt: latestImplementation.version,
    previousAudit,
    body,
    actorId: context.actorId,
    submittedAt,
  ...
  const result = await store.appendEvent({
    taskId,
    tenantId,
    eventType: 'task.contract_coverage_audit_submitted',
    actorId: context.actorId,
    actorType: body.actorType || 'user',
    idempotencyKey: body.idempotencyKey || `contract-coverage-audit:${taskId}:v${audit.contract_version}:i${audit.implementation_attempt}:a${audit.audit_version}`,
    occurredAt: submittedAt,
    payload: {
      audit_id: audit.audit_id,
      audit_version: audit.audit_version,
      contract_version: audit.contract_version,
      implementation_attempt: audit.implementation_attempt,
      coverage_audit: audit,
      readiness,
      waiting_state: 'contract_coverage_audit',
      next_required_action: CONTRACT_COVERAGE_AUDIT_ACTION,
    },
    source,
  });
  if (!result.duplicate) await recordContractCoverageAuditMetric(store, { submitted: 1 });
  return {
    result,
    audit: result.event?.payload?.coverage_audit || audit,
    readiness: result.event?.payload?.readiness || readiness,
  };
}

async function validateLatestContractCoverageAudit({ store, taskPlatform, taskId, tenantId, context, body = {}, source = 'http', logger }) {
  const { state, history, projection } = await loadExecutionContractContext(store, taskId, tenantId);
  assertApprovedContractForCoverage(projection, taskId);
  if (state.current_stage !== STAGES.CONTRACT_COVERAGE_AUDIT) {
    throw createHttpError(409, 'invalid_stage', 'Contract Coverage Audit validation can only run during Contract Coverage Audit.', {
      current_stage: state.current_stage,
    });
  }
  const coverageProjection = deriveContractCoverageAuditProjection(history, projection.latest);
  const audit = coverageProjection.latest;
  if (!audit) {
    throw createHttpError(409, 'contract_coverage_audit_missing', 'QA cannot validate until the Engineer submits the Contract Coverage Audit matrix.', {
      current_stage: state.current_stage,
    });
  }
  const validatedAt = new Date().toISOString();
  const { audit: validatedAudit, validation } = validateContractCoverageAudit({
    audit,
    contract: projection.latest,
    history,
    body,
  ...
  const result = await store.appendEvent({
    ...
    actorId: context.actorId,
    actorType: body.actorType || 'user',
    idempotencyKey: body.idempotencyKey || `contract-coverage-audit-validate:${taskId}:${audit.audit_id}`,
    ...
    payload: {
      ...
      contract_version: audit.contract_version,
      implementation_attempt: audit.implementation_attempt,
      coverage_audit: validatedAudit,
      validation,
      waiting_state: validation.status === 'closed' ? 'contract_coverage_audit_closed' : 'implementation_incomplete',
      next_required_action: validation.status === 'closed'
        ? CONTRACT_COVERAGE_AUDIT_CLOSED_ACTION
  ...
  if (!result.duplicate) {
    const signal = validation.autonomy_confidence_signal?.outcome;
    await recordContractCoverageAuditMetric(store, {
      closed: validation.status === 'closed' ? 1 : 0,
      implementationIncomplete: validation.status === 'closed' ? 0 : validation.blocking_exceptions.length || 1,
      positive: signal === 'positive' ? 1 : 0,
      neutral: signal === 'neutral' ? 1 : 0,
      negative: signal === 'negative' ? 1 : 0,
    });
  }
  if (!validation.gate_closed && !result.duplicate) {
    await store.appendEvent({
      taskId,
      tenantId,
      ...
      actorId: context.actorId,
      actorType: body.actorType || 'user',
      idempotencyKey: `contract-coverage-audit-return:${taskId}:${audit.audit_id}`,
      payload: {
        from_stage: STAGES.CONTRACT_COVERAGE_AUDIT,
        to_stage: STAGES.IMPLEMENTATION,
        agreement_artifact: audit.audit_id,
        rationale: 'Contract Coverage Audit found implementation_incomplete gaps.',
        waiting_state: 'implementation_incomplete',
        next_required_action: 'Address Contract Coverage Audit gaps and submit a new implementation attempt.',
        coverage_audit_id: audit.audit_id,
        blocking_exceptions: validation.blocking_exceptions,
      },
      source,
    });
    await trySyncCanonicalTask(taskPlatform, store, tenantId, taskId, logger, { route: 'contract_coverage_audit_returned_to_implementation' });
  }
  return {
    result,
    audit: result.event?.payload?.coverage_audit || validatedAudit,
    validation: result.event?.payload?.validation || validation,
  };
}
...
function createAuditApiServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      const v1MergeReadinessReviewsMatch = routePath.match(/^\/v1\/tasks\/([^/]+)\/merge-readiness-reviews$/);
      const v1TaskMatch = routePath.match(/^\/v1\/tasks\/([^/]+)$/);
      const v1TaskOwnerMatch = routePath.match(/^\/v1\/tasks\/([^/]+)\/owner$/);

      if (v1MergeReadinessReviewsMatch && req.method === 'GET') {
        const context = await requireTenantAccess(req, runtimeOptions);
        requirePermission(context, 'state:read');
        const currentOnly = url.searchParams.has('current')
          ? parseBooleanOption(url.searchParams.get('current'), true)
          : parseBooleanOption(url.searchParams.get('currentOnly'), true);
        const items = await withErrorHandling(async () => taskPlatform.listMergeReadinessReviews({
          tenantId: context.tenantId,
          taskId: v1MergeReadinessReviewsMatch[1],
          repository: url.searchParams.has('repository') ? url.searchParams.get('repository') : undefined,
          pullRequestNumber: url.searchParams.has('pullRequestNumber') ? url.searchParams.get('pullRequestNumber') : undefined,
          commitSha: url.searchParams.has('commitSha') ? url.searchParams.get('commitSha') : undefined,
          currentOnly,
        }))();
        return sendJson(res, 200, { data: { items, current: items.find((item) => item.isCurrent) || null } }, requestId);
      }

      if (v1MergeReadinessReviewsMatch && req.method === 'POST') {
        const context = await requireTenantAccess(req, runtimeOptions);
        requirePermission(context, 'events:write');
        const body = validateMergeReadinessReviewCreateRequest(await parseJson(req));
        const data = await withErrorHandling(async () => taskPlatform.createMergeReadinessReview({
          ...body,
          tenantId: context.tenantId,
          taskId: v1MergeReadinessReviewsMatch[1],
          actorId: context.actorId,
      ...
      if (v1MergeReadinessReviewsMatch && req.method === 'PATCH') {
        const context = await requireTenantAccess(req, runtimeOptions);
        requirePermission(context, 'events:write');
        const body = validateMergeReadinessReviewUpdateRequest(await parseJson(req));
        const data = await withErrorHandling(async () => taskPlatform.updateMergeReadinessReview({
          ...body,
          tenantId: context.tenantId,
          taskId: v1MergeReadinessReviewsMatch[1],
          actorId: context.actorId,
      ...
      if (routePath === '/deferred-considerations' && req.method === 'GET') {
        const context = await requireTenantAccess(req, runtimeOptions);
        requirePermission(context, 'state:read');
        if (!canManageDeferredConsiderations(context)) {
          throw createHttpError(403, 'forbidden', 'Only PM/admin may view the Deferred Considerations review queue.');
        }
        const queue = await listDeferredConsiderationReviewQueue({ store, tenantId: context.tenantId });
        logger.info({ feature: 'ff_audit_foundation', action: 'deferred_considerations_queue_read', outcome: 'success', request_id: requestId, method: req.method, path: url.pathname, tenant_id: context.tenantId, actor_id: context.actorId, result_count: queue.items.length, duration_ms: Date.now() - startedAt });
        return sendJson(res, 200, { success: true, data: queue }, requestId);
      }
      ...
      const contractCoverageAuditMatch = routePath.match(/^\/tasks\/([^/]+)\/contract-coverage-audit(?:\/(validate|markdown))?$/);
      const deferredConsiderationsMatch = routePath.match(/^\/tasks\/([^/]+)\/deferred-considerations(?:\/([^/]+)\/(review|promote|close))?$/);
      ...
      if (!summaryMatch && !resourceMatch && !assignmentMatch && !lockMatch && !skillEscalationMatch && !checkInsMatch && !retierMatch && !reassignmentMatch && !sreMonitoringMatch && !pmBusinessContextMatch && !closeReviewCancellationMatch && !closeReviewExceptionalDisputeMatch && !closeReviewHumanDecisionMatch && !closeReviewBacktrackMatch && !architectHandoffMatch && !engineerSubmissionMatch && !executionContractMatch && !contractCoverageAuditMatch && !deferredConsiderationsMatch && !orchestrationMatch && !workflowThreadsMatch && !workflowThreadActionMatch && !qaResultsMatch && !reviewQuestionsMatch && !reviewQuestionActionMatch) throw createHttpError(404, 'not_found', 'Route not found');
      ...
      const resource = assignmentMatch
        ...
        : executionContractMatch
          ...
          : contractCoverageAuditMatch
            ? `contract-coverage-audit${contractCoverageAuditMatch[2] ? `:${contractCoverageAuditMatch[2]}` : ''}`
          : deferredConsiderationsMatch
            ? `deferred-considerations${deferredConsiderationsMatch[3] ? `:${deferredConsiderationsMatch[3]}` : ''}`
          : orchestrationMatch
          ? 'orchestration'
      ...
      const deferredConsiderationId = deferredConsiderationsMatch ? deferredConsiderationsMatch[2] : null;
      const context = await requireTenantAccess(req, runtimeOptions);
      ...
      if (resource === 'execution-contract:approve' && req.method === 'POST') {
        ...
        await assertTaskUnlockedForMutation({ store, taskId, tenantId: context.tenantId, actorId: context.actorId, resource, options: runtimeOptions });
        const body = await parseJson(req);
        const { result, validation, contract, committedScope, approvalSummary, dispatchPolicy, autoApproval, autoApprovalPolicy } = await approveLatestExecutionContract({
          store,
          taskId,
          tenantId: context.tenantId,
          context,
      ...
      if (resource === 'contract-coverage-audit' && req.method === 'GET') {
        assertExecutionContractsEnabled(runtimeOptions);
        requirePermission(context, 'state:read');
        const { history, projection } = await loadExecutionContractContext(store, taskId, context.tenantId);
        assertApprovedContractForCoverage(projection, taskId);
        return sendJson(res, 200, {
          success: true,
          data: deriveContractCoverageAuditProjection(history, projection.latest),
        }, requestId);
      }
      if (resource === 'contract-coverage-audit' && req.method === 'POST') {
        assertExecutionContractsEnabled(runtimeOptions);
        requirePermission(context, 'events:write');
        if (!canSubmitContractCoverageAudit(context)) throw createHttpError(403, 'forbidden', 'Only engineer/admin may submit Contract Coverage Audit matrices.');
        await assertTaskUnlockedForMutation({ store, taskId, tenantId: context.tenantId, actorId: context.actorId, resource, options: runtimeOptions });
        const body = await parseJson(req);
        const { result, audit, readiness } = await submitContractCoverageAudit({
          store,
          taskId,
          tenantId: context.tenantId,
          context,
        ...
        await trySyncCanonicalTask(taskPlatform, store, context.tenantId, taskId, logger, { route: 'contract_coverage_audit_submitted' });
        return sendJson(res, result.duplicate ? 200 : 201, {
          success: true,
          data: {
            taskId,
            ...
            contractVersion: audit.contract_version,
            implementationAttempt: audit.implementation_attempt,
            status: audit.status,
            rows: audit.rows,
            requiredRequirements: audit.required_requirements,
            readiness,
            submittedAt: result.event.occurred_at,
          },
        }, requestId);
      }
      if (resource === 'contract-coverage-audit:validate' && req.method === 'POST') {
        assertExecutionContractsEnabled(runtimeOptions);
        requirePermission(context, 'events:write');
        if (!canValidateContractCoverageAudit(context)) throw createHttpError(403, 'forbidden', 'Only QA/admin may validate Contract Coverage Audit matrices.');
        await assertTaskUnlockedForMutation({ store, taskId, tenantId: context.tenantId, actorId: context.actorId, resource, options: runtimeOptions });
        const body = await parseJson(req);
        const { result, audit, validation } = await validateLatestContractCoverageAudit({
          store,
          taskPlatform,
          taskId,
          tenantId: context.tenantId,
        ...
        await trySyncCanonicalTask(taskPlatform, store, context.tenantId, taskId, logger, { route: 'contract_coverage_audit_validated' });
        return sendJson(res, result.duplicate ? 200 : 201, {
          success: true,
          data: {
            taskId,
            ...
            gateClosed: validation.gate_closed,
            canStartQaVerification: validation.can_start_qa_verification,
            blocksOperatorCloseout: validation.blocks_operator_closeout,
            blockingExceptions: validation.blocking_exceptions,
            autonomyConfidenceSignal: validation.autonomy_confidence_signal,
      ...
      if (resource === 'contract-coverage-audit:markdown' && req.method === 'GET') {
        assertExecutionContractsEnabled(runtimeOptions);
        requirePermission(context, 'state:read');
        const { history, projection } = await loadExecutionContractContext(store, taskId, context.tenantId);
        assertApprovedContractForCoverage(projection, taskId);
        const coverage = deriveContractCoverageAuditProjection(history, projection.latest);
        if (!coverage.validation?.markdown) {
          throw createHttpError(404, 'contract_coverage_audit_markdown_not_found', 'No Contract Coverage Audit Markdown view exists for the latest validation.', { task_id: taskId });
        }
        return sendJson(res, 200, { success: true, data: coverage.validation.markdown }, requestId);
      }

      if (resource === 'deferred-considerations' && req.method === 'GET') {
        requirePermission(context, 'state:read');
        const { projection } = await loadDeferredConsiderationContext(store, taskId, context.tenantId);
        return sendJson(res, 200, {
          success: true,
          data: projection,
        }, requestId);
      }
      if (resource === 'deferred-considerations' && req.method === 'POST') {
        if (!canManageDeferredConsiderations(context)) {
          throw createHttpError(403, 'forbidden', 'Only PM/admin may capture Deferred Considerations.');
        }
        await assertTaskUnlockedForMutation({ store, taskId, tenantId: context.tenantId, actorId: context.actorId, resource, options: runtimeOptions });
        const body = await parseJson(req);
        const { result, deferredConsideration } = await captureDeferredConsideration({
          store,
          taskId,
          tenantId: context.tenantId,
          context,
        ...
        await trySyncCanonicalTask(taskPlatform, store, context.tenantId, taskId, logger, { route: 'deferred_consideration_captured' });
        return sendJson(res, result.duplicate ? 200 : 201, {
          success: true,
          data: {
            taskId,
            deferredConsiderationId: deferredConsideration.deferred_consideration_id,
            status: deferredConsideration.status,
            deferredConsideration,
            capturedAt: result.event.occurred_at,
          },
        }, requestId);
      }
      if (resource === 'deferred-considerations:review' && req.method === 'POST') {
        if (!canOperateDeferredConsiderations(context)) {
          throw createHttpError(403, 'forbidden', 'Only PM, operator, stakeholder, or admin may review Deferred Considerations.');
        }
        await assertTaskUnlockedForMutation({ store, taskId, tenantId: context.tenantId, actorId: context.actorId, resource, options: runtimeOptions });
        const body = await parseJson(req);
        const { result, consideration, review, conversionEvent } = await reviewDeferredConsideration({
          store,
          taskId,
          tenantId: context.tenantId,
          context,
          considerationId: deferredConsiderationId,
          body,
        });
        await trySyncCanonicalTask(taskPlatform, store, context.tenantId, taskId, logger, { route: 'deferred_consideration_reviewed' });
        return sendJson(res, result.duplicate ? 200 : 202, {
          success: true,
          data: {
            taskId,
            deferredConsiderationId: consideration.id,
            status: review.status,
            review,
            conversionEventId: conversionEvent?.event?.event_id || null,
            reviewedAt: result.event.occurred_at,
          },
        }, requestId);
      }
      if (resource === 'deferred-considerations:promote' && req.method === 'POST') {
        if (!canOperateDeferredConsiderations(context)) {
          throw createHttpError(403, 'forbidden', 'Only PM, operator, stakeholder, or admin may promote Deferred Considerations.');
        }
        await assertTaskUnlockedForMutation({ store, taskId, tenantId: context.tenantId, actorId: context.actorId, resource, options: runtimeOptions });
        const body = await parseJson(req);
        const { result, consideration, promotedTaskId, promotionLink, rawRequirements } = await promoteDeferredConsideration({
          store,
          taskPlatform,
          taskId,
          tenantId: context.tenantId,
        ...
        await trySyncCanonicalTask(taskPlatform, store, context.tenantId, taskId, logger, { route: 'deferred_consideration_promoted' });
        return sendJson(res, result.duplicate ? 200 : 201, {
          success: true,
          data: {
            taskId,
            ...
            promotionLink,
            rawRequirements,
            promotedAt: result.event.occurred_at,
          },
        }, requestId);
      }
      if (resource === 'deferred-considerations:close' && req.method === 'POST') {
        if (!canOperateDeferredConsiderations(context)) {
          throw createHttpError(403, 'forbidden', 'Only PM, operator, stakeholder, or admin may close Deferred Considerations.');
        }
        await assertTaskUnlockedForMutation({ store, taskId, tenantId: context.tenantId, actorId: context.actorId, resource, options: runtimeOptions });
        const body = await parseJson(req);
        const { result, consideration, closePayload } = await closeDeferredConsideration({
          store,
          taskId,
          tenantId: context.tenantId,
          context,
        ...
        await trySyncCanonicalTask(taskPlatform, store, context.tenantId, taskId, logger, { route: 'deferred_consideration_closed' });
        return sendJson(res, result.duplicate ? 200 : 202, {
          success: true,
          data: {
            taskId,
            ...
            closedAt: result.event.occurred_at,
          },
        }, requestId);
      }
      ...
      if (resource === 'qa-results' && req.method === 'POST') {
        ...
        const escalationPackage = normalized.outcome === 'fail'
          ? buildQaEscalationPackage({ taskId, summary, architectHandoff, implementationHistory, latestFailedQa, executionContract, body: normalized, context })
          : null;
      ...
      if (resource === 'architect-handoff' && req.method === 'PUT') {
        ...
        const history = await store.getTaskHistory(taskId, { tenantId: context.tenantId, limit: 500 });
        const dispatchReadiness = evaluateApprovedContractDispatchForAssignment(history, {
          proposedEngineerTier: normalized.engineer_tier,
          proposedAssignee: buildTierAssigneeId(normalized.engineer_tier),
        });
        assertApprovedContractDispatchAllowed(dispatchReadiness);
        ...
        const result = await store.appendEvent({
          ...
          payload: {
            ...normalized,
            version,
            dispatch_policy: dispatchReadiness?.dispatchPolicy || null,
      ...
      if (resource === 'events' && req.method === 'POST') {
        ...
        try {
          ...
          if (body.eventType === 'task.closed') {
            ...
            if (!result.duplicate) {
              await maybeRecordOperatorTrustedAutonomousDelivery({
                store,
                taskId,
                tenantId: context.tenantId,
                closeEvent: result.event,
              });
            }
          }
      ...
      if (resource === 'assignment' && req.method === 'PATCH') {
        ...
        const shouldEvaluateEngineerDispatchPolicy = agent
          && inferCanonicalOwnerRole(agent.id) === 'engineer'
          && state.current_stage !== STAGES.DRAFT;
        const history = shouldEvaluateEngineerDispatchPolicy
          ? await store.getTaskHistory(taskId, { tenantId: context.tenantId, limit: 500 })
          : [];
        const dispatchReadiness = shouldEvaluateEngineerDispatchPolicy
          ? evaluateApprovedContractDispatchForAssignment(history, {
              proposedAssignee: agent.id,
              proposedEngineerTier: engineerTierFromAssigneeId(agent.id),
            })
          : null;
        assertApprovedContractDispatchAllowed(dispatchReadiness, createAssignmentError);
        ...
        return sendJson(res, 200, {
          ...
          data: {
            ...
            duplicate: result.duplicate,
            eventId: result.event.event_id,
            dispatchPolicy: dispatchReadiness?.dispatchPolicy || null,
          },
        }, requestId);
      }
