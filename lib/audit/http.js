...
function createAuditApiServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/projections/process' && req.method === 'POST') {
        const context = requireTenantAccess(req, options);
      ...
      const summaryMatch = url.pathname.match(/^\/tasks\/([^/]+)$/);
      const resourceMatch = url.pathname.match(/^\/tasks\/([^/]+)\/(events|history|state|relationships|observability-summary)$/);
      if (!summaryMatch && !resourceMatch) throw createHttpError(404, 'not_found', 'Route not found');
      const taskId = (summaryMatch || resourceMatch)[1];
      const resource = resourceMatch ? resourceMatch[2] : 'summary';
      const context = requireTenantAccess(req, options);
