...
function buildTaskDetailViewModel({ taskId, summary, relationships, history, telemetry, childTaskSummaries = [], context }) {
  ...
  const technicalSpec = lastDefinedPayloadValue(history, ['technical_spec', 'technicalSpec']);
  const monitoringSpec = lastDefinedPayloadValue(history, ['monitoring_spec', 'monitoringSpec']);
...
function createAuditApiServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (!summaryMatch && !resourceMatch && !assignmentMatch && !reviewQuestionsMatch && !reviewQuestionActionMatch) throw createHttpError(404, 'not_found', 'Route not found');
      const taskId = (summaryMatch || resourceMatch || assignmentMatch || reviewQuestionsMatch || reviewQuestionActionMatch)[1];
