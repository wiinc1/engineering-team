...
const DEFAULT_OPENCLAW_RUNNER = OPENCLAW_SPECIALIST_RUNNER;
const DEFAULT_GROK_RUNNER = GROK_SPECIALIST_RUNNER;
...
function resolveOpenClawDelegationRunner(env = process.env, baseDir = process.cwd()) {
  ...
  if (resolved.runner) return resolved.runner;
  return `node ${path.join(baseDir, 'scripts/grok-specialist-runner.js')}`;
}
...
function buildOpenClawPmRefinementEnv(env = process.env, baseDir = process.cwd()) {
  ...
  return {
    ...
    SPECIALIST_RUNTIME_PROVIDER: resolved.name,
    ...
    ...(env.GROK_SPECIALIST_MAP ? { GROK_SPECIALIST_MAP: env.GROK_SPECIALIST_MAP } : {}),
  };
}
