...
async function runProjectionCatchUp(ctx, options = {}) {
  ...
  if (ctx.baseUrl) {
    try {
      ...
      if (waitMs > 0) {
        ...
        if (lagAfterWait <= lagThresholdSeconds) {
          return {
            label,
            skipped: true,
            reason: 'worker_caught_up_after_wait',
            mode: 'always_on_worker',
            projectionLagSeconds: lagAfterWait,
            processed: 0,
            failed: 0,
          };
        }
      ...
      // Fall through to manual catch-up when metrics are unavailable.
    }
  }
