...
function orderLinkedPrHistoryEvents(history = []) {
  if (!Array.isArray(history)) return [];
  return [...history].sort((left, right) => {
    const leftSeq = Number(left?.sequence_number) || 0;
    const rightSeq = Number(right?.sequence_number) || 0;
    if (leftSeq !== rightSeq) return leftSeq - rightSeq;
    const leftAt = Date.parse(left?.occurred_at || left?.recorded_at || 0) || 0;
    const rightAt = Date.parse(right?.occurred_at || right?.recorded_at || 0) || 0;
...
function collectLinkedPrs(history = [], relationships = {}, taskId) {
  ...
  for (const event of orderLinkedPrHistoryEvents(history)) {
