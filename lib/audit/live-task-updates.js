...
function recordTenantId(record = {}) {
  return record.tenantId || record.tenant_id || record.payload?.tenantId || record.payload?.tenant_id || null;
}

function matchesRequestedTenant(record = {}, tenantId) {
  const recordTenant = recordTenantId(record);
  return !recordTenant || !tenantId || recordTenant === tenantId;
}
...
function taskUpdateFromRecords(summary, canonical, projectMutation) {
  ...
  const tenantId = task.tenant_id || recordTenantId(canonical) || recordTenantId(summary) || recordTenantId(projectMutation);
...
function projectUpdate(project = {}, projectMutation, tenantId = null) {
  const updatedAt = latestIso(
  ...
  return {
    ...
    tenantId: project.tenantId || project.tenant_id || recordTenantId(projectMutation) || tenantId,
...
async function readAllUpdates({ store, taskPlatform, tenantId }) {
  const [rawSummaries, rawCanonicalTasks, rawProjects, rawProjectMutations] = await Promise.all([
  ...
  const summaries = (rawSummaries || []).filter(record => matchesRequestedTenant(record, tenantId));
  const canonicalTasks = (rawCanonicalTasks || []).filter(record => matchesRequestedTenant(record, tenantId));
  const projects = rawProjects || [];
  const projectMutations = (rawProjectMutations || []).filter(record => matchesRequestedTenant(record, tenantId));
  ...
  return sortUpdates([
    ...taskUpdates,
    ...(projects || []).map(project => projectUpdate(project, latestProjectMutationByProjectId.get(project.projectId || project.project_id), tenantId)),
  ].filter(update => update.entityId));
}
