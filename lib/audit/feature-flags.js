...
function isIntakeDraftCreationEnabled(options = {}) {
  if (typeof options.intakeDraftCreationEnabled === 'boolean') return options.intakeDraftCreationEnabled;
  const raw = options.ffIntakeDraftCreation ?? process.env.FF_INTAKE_DRAFT_CREATION;
  if (raw === undefined || raw === null || raw === '') return true;
  return !['0', 'false', 'off', 'disabled', 'no'].includes(String(raw).trim().toLowerCase());
}

function assertIntakeDraftCreationEnabled(options = {}) {
  if (!isIntakeDraftCreationEnabled(options)) {
    const error = new Error('Intake draft creation is disabled by ff_intake_draft_creation');
    error.code = 'feature_disabled';
    error.statusCode = 503;
    error.details = { feature: 'ff_intake_draft_creation' };
    throw error;
  }
}

function isExecutionContractsEnabled(options = {}) {
  if (typeof options.executionContractsEnabled === 'boolean') return options.executionContractsEnabled;
  const raw = options.ffExecutionContracts ?? process.env.FF_EXECUTION_CONTRACTS;
  if (raw === undefined || raw === null || raw === '') return true;
  return !['0', 'false', 'off', 'disabled', 'no'].includes(String(raw).trim().toLowerCase());
}

function assertExecutionContractsEnabled(options = {}) {
  if (!isExecutionContractsEnabled(options)) {
    const error = new Error('Execution Contracts are disabled by ff_execution_contracts');
    error.code = 'feature_disabled';
    error.statusCode = 503;
    error.details = { feature: 'ff_execution_contracts' };
    throw error;
  }
}
