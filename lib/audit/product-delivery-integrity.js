...
function branchRefCandidates(branch) {
  const targetBranch = normalizeText(branch);
  if (!targetBranch) return [];
  const candidates = [targetBranch];
  if (!targetBranch.includes('/')) {
    candidates.push(`origin/${targetBranch}`);
  }
  return candidates;
}

function resolveDeclaredRunnableSurfaceHeadSha(options = {}) {
  const env = options.env || process.env;
  return normalizeText(env.RUNNABLE_SURFACE_HEAD_SHA) || null;
}

function shasMatch(candidate, reference) {
  const left = normalizeText(candidate).toLowerCase();
  const right = normalizeText(reference).toLowerCase();
...
function resolveBranchHead(branch, options = {}) {
  ...
  for (const ref of branchRefCandidates(branch)) {
    try {
      ...
      // Try the next ref for shallow CI checkouts that only fetch origin/<branch>.
    }
  }
  return resolveDeclaredRunnableSurfaceHeadSha(options);
}
...
function isCommitAncestorOfBranch(commitSha, branch, options = {}) {
  ...
  const declaredHead = resolveDeclaredRunnableSurfaceHeadSha(options);
  if (declaredHead && shasMatch(sha, declaredHead)) {
    return {
      verified: true,
      verified_branch: targetBranch,
      verified_branch_head: declaredHead,
      verified_at: new Date().toISOString(),
      mode: 'declared_runnable_surface_head',
    };
  }

  const repoRoot = resolveRepoRoot(options);
  for (const ref of branchRefCandidates(targetBranch)) {
    try {
