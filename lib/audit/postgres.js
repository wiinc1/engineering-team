...
const { linkedPrMatches } = require('./linked-prs');
...
function createPostgresAuditStore(options = {}) {
  ...
  async function updateMetrics(mutator, client = null) {
    const localClient = client || await pool.connect();
    try {
      const metrics = await readMetrics(localClient);
      await Promise.resolve(mutator(metrics));
      await writeMetrics(metrics, localClient);
      return metrics;
    } finally {
      if (!client) localClient.release();
  ...
  async function getTaskCurrentStateForMutation(taskId, filters = {}) {
    assertAuditFoundationEnabled(options);
    const projected = await getTaskCurrentState(taskId, filters);
    ...
    try {
      const result = await client.query(
        ...
        [taskId, filters.tenantId || null],
      );
      if (!result.rows.length) return null;

      return result.rows.reduce((state, event) => buildCurrentState(state, event), null);
    } finally {
      client.release();
    }
  }
  ...
  async function resolveTaskRefs(taskIds = [], filters = {}) {
    assertAuditFoundationEnabled(options);
    if (!taskIds.length) return [];
    const client = await pool.connect();
    try {
      await bumpMetric('workflow_current_state_queries_total', 1, client);
      const result = await client.query(
        'SELECT task_id, tenant_id FROM audit_task_current_state WHERE task_id = ANY($1::text[]) AND ($2::text IS NULL OR tenant_id = $2)',
        [taskIds, filters.tenantId || null],
      );
      return result.rows.map((row) => ({ taskId: row.task_id, tenantId: row.tenant_id }));
    } finally {
      client.release();
    }
  }
  ...
  async function listTaskSummaries(filters = {}) {
    ...
    try {
      await bumpMetric('workflow_current_state_queries_total', 1, client);
      const result = await client.query(
        `SELECT state.*, created.payload->>'title' AS created_title, created.payload->>'task_type' AS created_task_type
      ...
      return result.rows.map((state) => ({
        ...
        task_type: state.created_task_type || null,
  ...
  async function findTaskRefsByLinkedPr(pr, filters = {}) {
    assertAuditFoundationEnabled(options);
    const client = await pool.connect();
    try {
      await bumpMetric('workflow_relationship_queries_total', 1, client);
      const result = await client.query(
        'SELECT tenant_id, task_id, linked_prs FROM audit_task_relationships WHERE ($1::text IS NULL OR tenant_id = $1)',
        [filters.tenantId || null],
      );
      return result.rows
        .filter((row) => Array.isArray(row.linked_prs) && row.linked_prs.some((entry) => linkedPrMatches(entry, pr)))
        .map((row) => ({ taskId: row.task_id, tenantId: row.tenant_id }));
    } finally {
      client.release();
    }
  }
  ...
  async function applyEventToReadModels(event, client = pool) {
    ...
    await client.query(
      `INSERT INTO audit_task_relationships(tenant_id, task_id, child_task_ids, child_dependencies, escalations, decisions, linked_prs, github_sync, orchestration_state)
      ...
      [
        event.tenant_id,
        event.task_id,
        JSON.stringify(nextRelationships.child_task_ids),
        JSON.stringify(nextRelationships.child_dependencies || {}),
        JSON.stringify(nextRelationships.escalations),
        JSON.stringify(nextRelationships.decisions),
        JSON.stringify(nextRelationships.linked_prs || []),
        JSON.stringify(nextRelationships.github_sync || null),
        JSON.stringify(nextRelationships.orchestration_state || null),
      ],
    );
  }
  ...
  return {
    ...
    getTaskHistory,
    getTaskCurrentState,
    getTaskCurrentStateForMutation,
