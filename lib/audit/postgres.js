...
function createPostgresAuditStore(options = {}) {
  ...
  async function applyEventToReadModels(event, client = pool) {
    ...
    await client.query(
      `INSERT INTO audit_task_current_state(tenant_id, task_id, last_event_id, last_event_type, last_occurred_at, last_actor_id, current_stage, assignee, priority, blocked, closed)
      ...
      [nextState.tenant_id, nextState.task_id, nextState.last_event_id, nextState.last_event_type, nextState.last_occurred_at, nextState.last_actor_id, nextState.current_stage, nextState.assignee, nextState.priority, nextState.blocked, nextState.closed],
    );
    await client.query(
