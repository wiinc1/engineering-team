...
async function processGitHubPullRequestWebhook({
  ...
  const matchedTasks = await resolveWebhookTaskRefs(store, taskIds, linkedPr);
  ...
  const eventType = isPrComment ? 'task.github_pr_comment_recorded' : 'task.github_pr_synced';
