...
function existingIntakeResponse(existing, intakeBody, duplicate = false) {
...
function createdIntakeResponse({ deliveryId, result, tenantId, intakeBody }) {
  return {
    status: 201,
    body: {
      received: true,
...
async function createIssueIntakeDraft({
  ...
  if (result?.created?.duplicate) {
    return existingIntakeResponse({ taskId: result.createdTaskId, tenantId }, intakeBody, true);
  }
  logger?.info?.({
  ...
  return createdIntakeResponse({ deliveryId, result, tenantId, intakeBody });
}
...
async function processGitHubIssueIntakeWebhook({
  ...
  if (existing) return existingIntakeResponse(existing, intakeBody);

  return createIssueIntakeDraft({
    store, taskPlatform, createIntakeDraft, intakeBody, tenantId, options, requestId, deliveryId, logger,
  });
}
