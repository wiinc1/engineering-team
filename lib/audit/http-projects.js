...
const { createJobRuntimeRouteWrapper } = require('./job-runtime-http');

function createAuditApiServer(options = {}) {
  const consistency = preparePostgresHttpConsistency(options);
  const result = createJobRuntimeRouteWrapper(createLangGraphRouteWrapper(createReleaseHealthRouteWrapper(
    ...
    options,
    { getRequestContext, createIntakeDraftFromWebhook },
  ), options, { getRequestContext }), options, { getRequestContext });
