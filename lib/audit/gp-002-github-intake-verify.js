...
function resolveGp002Runtime(options = {}) {
...
function buildGp002Evidence(runtime, intakeProvider, issueNumber) {
  return {
    schemaVersion: '1.0',
...
function addGp002RuntimeChecks(evidence, { hostedProfile, runtime, stackState, stackStatePath }) {
  if (hostedProfile) {
...
async function runGp002WebhookChecks(runtime, options, evidence, secret, issueNumber, deliveryPrefix, intakeProvider) {
  ...
  evidence.intake = { createStatus: created.status, body: created.body, taskId };
  evidence.summary.checks.push({
  ...
  evidence.summary.checks.push({
    ...
    ok: duplicate.status === 202 && duplicate.body?.reason === 'existing_intake_task' && duplicate.body?.taskId === taskId,
    status: duplicate.status,
    taskId: duplicate.body?.taskId || null,
  });
  return { created, duplicate, taskId };
}

async function runGp002ProjectionCheck(runtime, options, evidence, taskId) {
  ...
  evidence.projection = { ok: projection.ok, projected: projection.projected, attempts: projection.attempt };
  evidence.summary.checks.push({
...
async function runGp002HistoryCheck(runtime, options, evidence, taskId, issueNumber, expectedIssueUrl) {
  const history = taskId ? await fetchTaskHistory(runtime, taskId, runtime.jwtSecret) : { status: null, ok: false, body: {} };
...
function resolveGp002ArtifactPaths(options, outputDir) {
  return {
    smokePath: options.smokePath || path.join(outputDir, 'gp-002-github-intake-smoke.json'),
    completePath: options.completePath || path.join(outputDir, 'gp-002-complete.json'),
    verifyPath: options.verifyPath || path.join(outputDir, 'gp-002-github-intake-verify.json'),
    canonicalSmokePath: options.canonicalSmokePath || 'observability/gp-002-github-intake-smoke.json',
  };
}

function buildGp002Smoke(evidence, runtime, issueNumber, created, taskId) {
  return {
    schemaVersion: '1.0',
...
function buildGp002Complete(evidence, paths, created, taskId, intakeProvider) {
  return {
    schemaVersion: '1.0',
    ...
    artifacts: {
      verify: path.relative(process.cwd(), path.resolve(paths.verifyPath)),
      smoke: path.relative(process.cwd(), path.resolve(paths.smokePath)),
      canonicalSmoke: paths.canonicalSmokePath,
    },
    notes: [
...
function writeGp002Artifacts(paths, evidence, smoke, complete) {
  fs.writeFileSync(path.resolve(paths.verifyPath), `${JSON.stringify(evidence, null, 2)}\n`);
  fs.writeFileSync(path.resolve(paths.smokePath), `${JSON.stringify(smoke, null, 2)}\n`);
  fs.writeFileSync(path.resolve(paths.completePath), `${JSON.stringify(complete, null, 2)}\n`);
  fs.mkdirSync(path.dirname(path.resolve(paths.canonicalSmokePath)), { recursive: true });
  fs.copyFileSync(path.resolve(paths.smokePath), path.resolve(paths.canonicalSmokePath));
  evidence.artifacts.verify = paths.verifyPath;
  evidence.artifacts.smoke = paths.smokePath;
  evidence.artifacts.complete = paths.completePath;
  evidence.artifacts.canonicalSmoke = paths.canonicalSmokePath;
}

async function runGp002GithubIntakeVerify(options = {}) {
  const runtime = resolveGp002Runtime(options);
  ...
  const evidence = buildGp002Evidence(runtime, intakeProvider, issueNumber);
  addGp002RuntimeChecks(evidence, { hostedProfile, runtime, stackState, stackStatePath });
  const { created, taskId } = await runGp002WebhookChecks(
    runtime, options, evidence, secret, issueNumber, deliveryPrefix, intakeProvider,
  );
  await runGp002ProjectionCheck(runtime, options, evidence, taskId);
  await runGp002HistoryCheck(runtime, options, evidence, taskId, issueNumber, expectedIssueUrl);
  evidence.summary.passed = evidence.summary.checks.every((check) => check.ok);
  const paths = resolveGp002ArtifactPaths(options, outputDir);
  const smoke = buildGp002Smoke(evidence, runtime, issueNumber, created, taskId);
  const complete = buildGp002Complete(evidence, paths, created, taskId, intakeProvider);
  writeGp002Artifacts(paths, evidence, smoke, complete);
