...
async function runAuditWorkersProductionSmoke(options = {}) {
  ...
  const create = await fetchJson(ctx.fetchImpl, `${ctx.baseUrl.replace(/\/+$/, '')}/tasks/${encodeURIComponent(taskId)}/events`, {
    method: 'POST',
  ...
  evidence.summary.checks.push({
    name: 'append_event',
    ok: create.status === 202,
    status: create.status,
  });

  const lagBefore = await readProjectionLagSeconds(ctx.baseUrl, authHeaders).catch((error) => {
  ...
  const lagAfter = await readProjectionLagSeconds(ctx.baseUrl, authHeaders).catch((error) => {
    evidence.metrics.lagReadErrorAfter = error.message;
    return null;
  });
  evidence.metrics.projectionLagSecondsAfter = lagAfter;

  const lagOk = lagAfter != null && lagAfter <= ctx.lagThresholdSeconds;
  evidence.summary.checks.push({
  ...
  const state = await fetchJson(
    ctx.fetchImpl,
    `${ctx.baseUrl.replace(/\/+$/, '')}/tasks/${encodeURIComponent(taskId)}/state`,
    { headers: authHeaders },
  );
