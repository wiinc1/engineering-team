...
async function runGithubIntakeSmoke(runtime, outputPath) {
  ...
  const { githubSignature, writeEvidence } = require('../../scripts/golden-path-smoke-lib');
  const issueNumber = 900_000 + Math.floor(Math.random() * 99_000);
  const secret = runtime.githubWebhookSecret || 'golden-path-local-webhook-secret';
  const payload = {
    action: 'opened',
    issue: {
      number: issueNumber,
      title: 'Milestone A staging intake smoke',
      body: 'Automated milestone-A GP-002 smoke.',
  ...
  const response = await runtime.fetchImpl(`${runtime.baseUrl.replace(/\/+$/, '')}/github/webhooks`, {
    ...
    headers: {
      ...
      'x-github-delivery': `milestone-a-gp-002-${issueNumber}`,
      'x-hub-signature-256': githubSignature(secret, body),
    },
    body,
  });
  const responseBody = await response.json().catch(() => ({}));
  const passed = response.ok && Boolean(
    responseBody.taskId || responseBody.task_id || responseBody.existing_intake_task,
  );
