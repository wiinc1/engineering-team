...
const { collectLinkedPrs, linkedPrMatches, openLinkedPrs } = require('./linked-prs');
const {
...
function lastQaResultFromHistory(history = []) {
...
function isIntakeDraftHistory(history = []) {
...
function isProtectedIntakeDraftState(state, history = []) {
  if (!isIntakeDraftHistory(history)) return false;
  const currentStage = state?.current_stage || STAGES.BACKLOG;
  return currentStage === STAGES.DRAFT
    || state?.waiting_state === 'task_refinement'
    || state?.next_required_action === 'PM refinement required';
}

function isImplementationPreparationTransition(fromStage, toStage) {
  return (
    (fromStage === STAGES.DRAFT && toStage === STAGES.BACKLOG)
    || toStage === STAGES.IMPLEMENTATION
    || toStage === STAGES.IN_PROGRESS
  );
}

function assertExecutionContractDispatchReadiness({ history = [], fromStage, toStage }) {
  if (!isImplementationPreparationTransition(fromStage, toStage)) return;
  const projection = deriveExecutionContractProjection(history);
  if (!projection.active || !projection.approval) return;
  ...
  if (!readiness.canDispatch) {
    const primaryBlocker = readiness.blockedReasons?.[0]?.detail;
    throw new WorkflowError(primaryBlocker || 'Implementation dispatch is blocked until the approved Execution Contract dispatch gates pass.');
  }
}

function isEngineerAssignee(value) {
  const normalized = String(value || '').trim().toLowerCase();
  return normalized === 'engineer' || normalized.startsWith('engineer-');
}

function assertExecutionContractAssignmentDispatchPolicy({ history = [], state = {}, assignee = null }) {
  if (!isEngineerAssignee(assignee) || state?.current_stage === STAGES.DRAFT) return;
  const projection = deriveExecutionContractProjection(history);
  if (!projection.active || !projection.approval) return;
  const readiness = evaluateExecutionContractDispatchReadiness({
    contract: projection.latest,
    ...
    proposedEngineerTier: engineerTierFromAssigneeId(assignee),
  });
  if (!readiness.canDispatch) {
    const primaryBlocker = readiness.blockedReasons?.[0]?.detail;
    throw new WorkflowError(primaryBlocker || 'Execution Contract dispatch policy blocked this assignment.');
  }
}

function approvedExecutionContractProjection(history = []) {
  const projection = deriveExecutionContractProjection(history);
  return projection.active && projection.approval ? projection : null;
}

function currentImplementationAttempt(history = []) {
  const submission = lastEngineerSubmissionFromHistory(history);
  return Number(submission?.payload?.version) || 0;
}

function assertContractCoverageAuditGate({ history = [], fromStage, toStage }) {
  const projection = approvedExecutionContractProjection(history);
  if (!projection) return;
  const implementationAttempt = currentImplementationAttempt(history);
  const coverage = deriveContractCoverageAuditProjection(history, projection.latest);
  const latestAudit = coverage.latest || null;
  const latestValidation = coverage.validation || null;

  if ([STAGES.IMPLEMENTATION, STAGES.IN_PROGRESS].includes(fromStage) && toStage === STAGES.CONTRACT_COVERAGE_AUDIT) {
    if (!implementationAttempt) {
      throw new WorkflowError('Contract Coverage Audit cannot begin until the Engineer submits implementation metadata.');
    }
    if (!latestAudit || Number(latestAudit.implementation_attempt) !== implementationAttempt) {
      throw new WorkflowError('Contract Coverage Audit matrix is required for the current implementation attempt before leaving Implementation.');
    }
  }

  if (
    ([STAGES.IMPLEMENTATION, STAGES.IN_PROGRESS, STAGES.CONTRACT_COVERAGE_AUDIT].includes(fromStage))
    && toStage === STAGES.QA_TESTING
  ) {
    if (!latestValidation || latestValidation.status !== 'closed' || Number(latestValidation.implementation_attempt) !== implementationAttempt) {
      const blocker = latestValidation?.blocking_exceptions?.[0]?.detail;
      throw new WorkflowError(blocker || 'QA Verification is blocked until Contract Coverage Audit is complete for the current implementation attempt.');
    }
  }
}

function assertContractCoverageCloseoutAllowed(history = []) {
  const projection = approvedExecutionContractProjection(history);
  if (!projection) return;
  const coverage = deriveContractCoverageAuditProjection(history, projection.latest);
  const validation = coverage.validation || null;
  if (!validation || validation.status !== 'closed' || validation.blocks_operator_closeout) {
    const blocker = validation?.blocking_exceptions?.[0]?.detail;
    throw new WorkflowError(blocker || 'Operator Closeout is blocked while committed requirements remain uncovered by Contract Coverage Audit.');
  }
}
...
function updateControlPlaneMetrics(store, mutator) {
  if (typeof store.updateMetrics !== 'function') return;
  store.updateMetrics((metrics) => {
    mutator(metrics);
  });
}

function normalizeControlPlaneDecisionInput(input) {
  input.payload = input.payload || {};
  const decisionSource = input.payload.control_plane_decision || input.payload.policy_decision || input.payload;
  input.payload.control_plane_decision = normalizePolicyDecision(decisionSource, {
...
function normalizeControlPlaneExceptionInput(input) {
  input.payload = input.payload || {};
  const exceptionSource = input.payload.exception || input.payload.control_plane_exception || input.payload;
  input.payload.exception = normalizeException(exceptionSource, {
    task_id: input.taskId,
    event_type: input.eventType,
  });
  if (input.payload.exception.blocked_state) {
    input.payload.blocked = true;
    input.payload.waiting_state = input.payload.waiting_state || 'workflow_exception';
    input.payload.next_required_action = input.payload.next_required_action || 'Resolve linked workflow exception before continuing.';
  }
}

async function enrichStageChangeWithWipPolicy({ store, input, state }) {
  const controlPlane = input.payload?.control_plane || input.payload?.controlPlane || {};
  const policyInput = controlPlane.wipLimits || controlPlane.wip_limits || input.payload?.wipLimits || input.payload?.wip_limits;
  if (!policyInput) return;
  const obligations = Array.isArray(policyInput.obligations) ? policyInput.obligations : [];
  const evaluations = evaluateWipLimits({
    ...policyInput,
    obligations,
    actorId: input.actorId,
    timestamp: input.occurredAt || new Date().toISOString(),
    taskRisk: policyInput.taskRisk || policyInput.task_risk || {
      production_incident: input.payload?.production_incident,
      s1_security_data_risk: input.payload?.s1_security_data_risk,
    },
    currentCount: policyInput.currentCount ?? policyInput.current_count ?? state?.wip_count ?? 0,
  });
  input.payload.control_plane_wip_decision = evaluations;
  if (evaluations.blocked) {
    const blocked = evaluations.evaluations.find((entry) => entry.blocked);
    updateControlPlaneMetrics(store, (metrics) => {
      metrics.feature_control_plane_wip_would_block_total += evaluations.would_block_count;
      metrics.feature_control_plane_wip_blocks_total += 1;
    });
    throw new WorkflowError(blocked?.rationale || 'WIP enforcement blocked this transition.');
  }
  return (metrics) => {
    metrics.feature_control_plane_wip_would_block_total += evaluations.would_block_count;
  };
...
function enrichWithBudgetPolicy({ input }) {
  const controlPlane = input.payload?.control_plane || input.payload?.controlPlane || {};
  const budgetInput = controlPlane.budget || controlPlane.budgets || input.payload?.delivery_budget || input.payload?.deliveryBudget;
  ...
  input.payload.control_plane_budget_decision = decision;
  if (decision.workflow_exception) {
    input.payload.workflow_exception = decision.workflow_exception;
    input.payload.waiting_state = input.payload.waiting_state || 'workflow_exception';
    input.payload.next_required_action = input.payload.next_required_action || decision.next_required_action;
    return (metrics) => {
      metrics.feature_control_plane_budget_exhausted_total += 1;
    };
  }
}
...
function createAdapterBackedPostgresStore(options = {}) {
  ...
  return {
    ...
    async getTaskCurrentStateForMutation(taskId, filters = {}) {
      assertAuditFoundationEnabled(options);
      return typeof pool.getTaskCurrentStateForMutation === 'function'
        ? pool.getTaskCurrentStateForMutation(taskId, filters)
        : pool.getTaskCurrentState(taskId, filters);
    },
    ...
    async resolveTaskRefs(taskIds = [], filters = {}) { assertAuditFoundationEnabled(options); return pool.resolveTaskRefs(taskIds, filters); },
    async findTaskRefsByLinkedPr(pr, filters = {}) { assertAuditFoundationEnabled(options); return pool.findTaskRefsByLinkedPr(pr, filters); },
...
function createFileAuditStore(options = {}) {
  ...
  function resolveTaskRefs(taskIds = [], filters = {}) {
    assertAuditFoundationEnabled(options);
    const metrics = readMetricsWithLag();
    metrics.workflow_current_state_queries_total += 1;
    writeMetrics(metrics);
    const wanted = new Set(taskIds.map((taskId) => String(taskId || '').trim()).filter(Boolean));
    if (!wanted.size) return [];
    return Object.values(readJson(files.currentState, {}))
      .filter((state) => wanted.has(state.task_id) && matchesTenant(state, filters.tenantId))
      .map((state) => ({ taskId: state.task_id, tenantId: state.tenant_id }));
  }

  function findTaskRefsByLinkedPr(pr, filters = {}) {
    assertAuditFoundationEnabled(options);
    const metrics = readMetricsWithLag();
    metrics.workflow_relationship_queries_total += 1;
    writeMetrics(metrics);
    return Object.values(readJson(files.relationships, {}))
      .filter((relationship) => matchesTenant(relationship, filters.tenantId))
      .filter((relationship) => Array.isArray(relationship.linked_prs) && relationship.linked_prs.some((entry) => linkedPrMatches(entry, pr)))
      .map((relationship) => ({ taskId: relationship.task_id, tenantId: relationship.tenant_id }));
  }
  ...
  function listTaskSummaries(filters = {}) {
    ...
    return states.map((state) => {
      ...
      const queueEvent = [...history].reverse().find((event) => ['task.assigned', 'task.reassigned', 'task.priority_changed', 'task.stage_changed', 'task.escalated', 'task.decision_recorded'].includes(event.event_type)) || latestEvent || createdEvent || null;
      return {
  ...
  return {
    ...
    getTaskHistory,
    getTaskCurrentState,
    getTaskCurrentStateForMutation: getTaskCurrentState,
    getTaskCurrentStates,
    resolveTaskRefs,
    findTaskRefsByLinkedPr,
    listTaskSummaries,
...
function createAuditStore(options = {}) {
  ...
  if (isWorkflowEngineEnabled(options)) {
    ...
    const getTaskCurrentStateForMutation = (taskId, tenantId) => (
      typeof store.getTaskCurrentStateForMutation === 'function'
    ...
    store.appendEvent = async function(input) {
      const controlPlaneMetricUpdates = [];
      if (input.eventType === 'task.control_plane_decision_recorded') {
        normalizeControlPlaneDecisionInput(input);
        controlPlaneMetricUpdates.push((metrics) => {
          metrics.feature_control_plane_decisions_total += 1;
        });
      }

      if (input.eventType === 'task.control_plane_exception_recorded') {
        normalizeControlPlaneExceptionInput(input);
        controlPlaneMetricUpdates.push((metrics) => {
          metrics.feature_control_plane_exceptions_total += 1;
        });
      }

      if (input.payload) {
        const budgetMetricUpdate = enrichWithBudgetPolicy({ input });
        if (budgetMetricUpdate) controlPlaneMetricUpdates.push(budgetMetricUpdate);
      }

      if (input.eventType === 'task.stage_changed') {
        const state = await getTaskCurrentStateForMutation(input.taskId, input.tenantId);
        ...
        const wipMetricUpdate = await enrichStageChangeWithWipPolicy({ store, input, state });
        if (wipMetricUpdate) controlPlaneMetricUpdates.push(wipMetricUpdate);
        ...
        if (isExecutionContractsEnabled(options)) {
          assertExecutionContractDispatchReadiness({ history, fromStage, toStage });
          assertContractCoverageAuditGate({ history, fromStage, toStage });
        }

        if (fromStage === STAGES.DRAFT && isIntakeDraftHistory(history)) {
          const projection = deriveExecutionContractProjection(history);
          if (!projection.approval) {
            throw new WorkflowError('Intake Drafts require PM refinement before workflow advancement.');
          }
        }
        ...
        if (
          ...
          if (toStage === STAGES.SRE_MONITORING && qaResult.payload?.outcome !== 'pass') {
            throw new WorkflowError('SRE monitoring handoff requires a passing QA result.');
          }
          if (toStage === STAGES.IMPLEMENTATION && qaResult.payload?.outcome !== 'fail') {
            throw new WorkflowError('Returning work to implementation requires a failing QA result.');
          }
        }
      ...
      if (input.eventType === 'task.closed') {
        ...
        if (isGitHubSyncEnabled(options)) {
          ...
          if (blockingPrs.length) {
            ...
            throw new WorkflowError(`Close is blocked until linked pull requests are merged or closed. Still open: ${refs}`);
          }
        }
        if (isExecutionContractsEnabled(options)) {
          assertContractCoverageCloseoutAllowed(history);
        }
        if (!input.payload?.delivery_retrospective_signal) {
          input.payload = {
            ...(input.payload || {}),
            delivery_retrospective_signal: generateDeliveryRetrospectiveSignal({
              taskId: input.taskId,
              history,
              state,
              closedEvent: {
              ...
              generatedAt: input.occurredAt || new Date().toISOString(),
            }),
          };
          controlPlaneMetricUpdates.push((metrics) => {
            metrics.feature_control_plane_delivery_retrospective_signals_total += 1;
          });
        }
      ...
      if (String(input.eventType || '').startsWith('task.execution_contract_')) {
        if (!isExecutionContractsEnabled(options)) {
          throw new WorkflowError('Execution Contracts are disabled.');
        }
        const state = await getTaskCurrentStateForMutation(input.taskId, input.tenantId);
        const history = await store.getTaskHistory(input.taskId, { tenantId: input.tenantId, limit: 500 });
        if (!state) {
          throw new WorkflowError('Execution Contracts can only be recorded for an existing Intake Draft task.');
        }
        if (!isIntakeDraftHistory(history)) {
          throw new WorkflowError('Execution Contracts can only be generated from an Intake Draft.');
        }
        if (state.current_stage !== STAGES.DRAFT) {
          throw new WorkflowError('Execution Contracts can only be refined while the task is in Draft.');
        }
      }
      ...
      if (input.eventType === 'task.architect_handoff_recorded') {
        ...
        if (isExecutionContractsEnabled(options)) {
          const history = await store.getTaskHistory(input.taskId, { tenantId: input.tenantId, limit: 500 });
          assertExecutionContractAssignmentDispatchPolicy({
            history,
            state,
      ...
      if (input.eventType === 'task.contract_coverage_audit_submitted') {
        if (!isExecutionContractsEnabled(options)) {
          throw new WorkflowError('Execution Contracts are disabled.');
        }
        const state = await getTaskCurrentStateForMutation(input.taskId, input.tenantId);
        const currentStage = state?.current_stage || STAGES.BACKLOG;
        if (![STAGES.IMPLEMENTATION, STAGES.IN_PROGRESS].includes(currentStage)) {
          throw new WorkflowError('Contract Coverage Audit can only be submitted during Implementation.');
        }
      }

      if (input.eventType === 'task.contract_coverage_audit_validated') {
        if (!isExecutionContractsEnabled(options)) {
          throw new WorkflowError('Execution Contracts are disabled.');
        }
        const state = await getTaskCurrentStateForMutation(input.taskId, input.tenantId);
        const currentStage = state?.current_stage || STAGES.BACKLOG;
        if (currentStage !== STAGES.CONTRACT_COVERAGE_AUDIT) {
          throw new WorkflowError('Contract Coverage Audit validation can only be recorded during Contract Coverage Audit.');
        }
      }

      if (['task.assigned', 'task.reassigned', 'task.unassigned'].includes(input.eventType)) {
        const state = await getTaskCurrentStateForMutation(input.taskId, input.tenantId);
        const history = await store.getTaskHistory(input.taskId, { tenantId: input.tenantId, limit: 500 });
        if (isProtectedIntakeDraftState(state, history)) {
          throw new WorkflowError('Intake Drafts must remain assigned to PM until refinement completes.');
        }
        if (isExecutionContractsEnabled(options) && ['task.assigned', 'task.reassigned'].includes(input.eventType)) {
          assertExecutionContractAssignmentDispatchPolicy({
            history,
            state,
            assignee: input.payload?.assignee,
      ...
      if ([
        ...
        if (!isReassignmentGhostingEnabled(options)) {
          throw new WorkflowError('Reassignment and ghosting workflow is disabled.');
        }
      }

      if (input.eventType?.startsWith('task.workflow_thread_')) {
        if (!isStructuredCommentsEnabled(options)) {
          throw new WorkflowError('Structured comments are disabled.');
        }
      }

      if (input.eventType === 'task.qa_result_recorded') {
        if (!isQaStageEnabled(options)) {
          throw new WorkflowError('QA stage is disabled.');
        }
        if (!isQaContextRoutingEnabled(options)) {
          throw new WorkflowError('QA context routing is disabled.');
        }
        const state = await getTaskCurrentStateForMutation(input.taskId, input.tenantId);
        const currentStage = state?.current_stage || STAGES.BACKLOG;
      ...
      const result = await originalAppendEvent.call(store, input);
      if (!result?.duplicate && controlPlaneMetricUpdates.length) {
        updateControlPlaneMetrics(store, (metrics) => {
          for (const update of controlPlaneMetricUpdates) update(metrics);
        });
      }
      return result;
    };
