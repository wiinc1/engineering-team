...
function assertGithubProjectApis(taskPlatform) {
...
function buildGithubBootstrapContext(payload = {}, intakeBody = {}, options = {}) {
  ...
  return {
    ...
    issue,
    issueNumber,
    githubIssueUrl: intakeBody.githubIssueUrl || issue.html_url || null,
    repository,
    labels: issueLabels(payload),
    projectIdempotencyKey: repository && issueNumber != null
...
async function createGithubProject(taskPlatform, tenantId, context, requestId) {
  const projectName = buildProjectName(context.issueNumber, context.issue.title);
  return taskPlatform.createProject({
    tenantId,
    actorId: context.actorId,
    name: projectName,
    summary: String(context.issue.title || '').trim() || 'Factory intake project bootstrap',
    status: 'ACTIVE',
    ownerActorId: 'pm',
    metadata: {
      ...
      childIssue: context.issueNumber ?? null,
      githubIssueUrl: context.githubIssueUrl,
      github_issue_url: context.githubIssueUrl,
...
function logGithubBootstrap(logger, { requestId, taskId, tenantId, project, context, created, attach }) {
  logger?.info?.({
...
async function bootstrapProjectForIssueIntake({
  ...
  if (!isGitHubIntakeProjectBootstrapEnabled(options)) return { skipped: true, reason: 'feature_disabled' };
  ...
  assertGithubProjectApis(taskPlatform);
  const context = buildGithubBootstrapContext(payload, intakeBody, options);
  const snapshot = await readTaskSnapshot(taskPlatform, tenantId, taskId);
  const existing = existingProjectResult(snapshot, taskId);
  if (existing) return existing;

  let project = await findProjectByGithubIssueUrl(taskPlatform, tenantId, context.githubIssueUrl);
  let created = false;
  if (!project) {
    project = await createGithubProject(taskPlatform, tenantId, context, requestId);
    created = true;
  }
  ...
  logGithubBootstrap(logger, { requestId, taskId, tenantId, project, context, created, attach });
