...
- Role policy is still hard-coded until a versioned policy layer exists. The future role-policy boundary should own supported role keys and labels, role inbox eligibility, PM overview bucket eligibility, assignment eligibility and fallback behavior, delegation specialist family mapping, role-specific permissions, and metrics/reporting dimensions. Until then, unsupported roles are rejected or filtered from live assignment controls, inbox filters, PM buckets, delegation routes, dashboards, metrics, and fallback-sensitive workflow paths.
