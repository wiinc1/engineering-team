const { createTaskDetailApiClient } = require('./adapter');

const DEFAULT_TASK_DETAIL_TAB = 'history';
...
function createTaskDetailPageModule({ client = createTaskDetailApiClient() } = {}) {
  return {
    match(pathname) {
    ...
    async load({ pathname, search = '' }) {
      ...
      try {
        const model = await client.fetchTaskDetailScreenData(route.taskId, {
        ...
        return {
          ...model,
          route: {
            pathname: `/tasks/${encodeURIComponent(route.taskId)}`,
            taskId: route.taskId,
          },
          shell: {
            ...model.shell,
