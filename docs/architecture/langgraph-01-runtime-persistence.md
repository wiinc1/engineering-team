...
Runtime latency histograms append observations in amortized constant time and clone samples only at the
snapshot boundary. This prevents telemetry collection itself from creating quadratic framework overhead
during the ten-minute load gate while keeping emitted snapshots immutable from later observations.
