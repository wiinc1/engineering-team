...
- TSK-001 Command Center closeout: `/tasks` uses page title `Command Center`, queue toolbar `Queue-first task workspace`, and a nested inspector `role="region"` (`aria-label=Selected task inspector`) instead of a top-level `aside` landmark.
