...
- in async projection environments, the same owner save performs a bounded projection drain before returning; immediately reloading `/tasks/<task-id>/detail` should show owner `pm`, waiting state `Task refinement`, and next action `PM refinement required`
...
- for delegation-enabled AI agents, verify `POST /api/v1/ai-agents/preview` returns a passing OpenClaw dry-run, no route/task-type collisions, `fallbackBehavior.failClosed=true`, `permissionsImpact.requiredToPreview` includes `agent-delegation:write`, and no coordinator fallback before sending a live create/update confirmation
