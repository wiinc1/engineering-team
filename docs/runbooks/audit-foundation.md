...
- Vercel production routes the nested versioned task workflow actions through `api/v1/task-workflow-proxy.js`: `/api/v1/tasks/{taskId}/execution-contract/{action}`, `/api/v1/tasks/{taskId}/contract-coverage-audit/{action}`, and `/api/v1/tasks/{taskId}/sre-monitoring/{action}`. `__workflow_path` is an internal rewrite parameter only; use `/api/v1/...` or `/backend/v1/...` for validation while preserving shared-handler JWT/RBAC enforcement.
