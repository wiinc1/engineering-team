...
17. Confirm `committed_scope.committed_requirements` contains only approved implementation scope, while `out_of_scope`, `deferred_considerations`, and `follow_up_tasks` remain excluded. Confirm GitHub issue creation stayed default-off unless explicitly requested.
18. Confirm Standard-or-higher and risk-bearing Simple tasks cannot enter implementation preparation before the skeleton event, while Simple no-risk tasks are allowed to proceed without one.
