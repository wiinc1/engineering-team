...
function passedArtifact(artifactName, name) {
  return {
...
function buildArtifacts(dir, healthOverrides = {}) {
  return {
    build: writeJson(path.join(dir, 'build.json'), passedArtifact('build', 'build')),
    compatibility: writeJson(path.join(dir, 'compatibility-report.json'), passedArtifact('compatibility-report', 'unit tests')),
    vulnerability: writeJson(path.join(dir, 'vulnerability-scan.json'), passedArtifact('vulnerability-scan', 'Dependency vulnerability scan')),
    secret: writeJson(path.join(dir, 'secret-scan.json'), passedArtifact('secret-scan', 'Secret scan')),
...
function realEvidence(dir, healthOverrides = {}) {
  return {
    ...
    releaseEvidence: {
      environment: 'staging',
      artifacts: buildArtifacts(dir, healthOverrides),
      validation: { ok: true, skipped: false },
    },
  };
}

test('real autonomous delivery audit requires health commit verification for hosted release proof', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'real-delivery-health-commit-'));
  const result = verifyRealAutonomousDeliveryEvidence({
    repoRoot: tmp,
    evidence: realEvidence(tmp, { commit_verified: false }),
    requireCandidateProof: false,
  });
  assert.equal(result.ok, false);
  assert.match(result.failures.join('\n'), /post-deploy-health\.commit_verified must be true/);
});
