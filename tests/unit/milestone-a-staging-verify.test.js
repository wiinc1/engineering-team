...
test('milestone A staging verify aggregates worker, bridge, intake, and factory checks', async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (route.endsWith('/metrics')) {
      return {
        ...
        text: async () => [
          ...
          'workflow_projection_events_processed_total 12',
          'workflow_outbox_events_published_total 8',
        ].join('\n') + '\n',
      };
    }
    ...
    if (route.endsWith('/tasks') && method === 'POST' && !route.includes('/api/v1/')) {
      return {
        ok: true,
        status: 201,
        json: async () => ({ taskId: 'TSK-INTAKE-SMOKE', task_id: 'TSK-INTAKE-SMOKE' }),
        text: async () => JSON.stringify({ taskId: 'TSK-INTAKE-SMOKE', task_id: 'TSK-INTAKE-SMOKE' }),
      };
    }
    ...
    if (route.includes('/tasks/') && route.endsWith('/state')) {
      return {
        ...
        text: async () => JSON.stringify({
          data: {
            task_id: route.includes('TSK-INTAKE-SMOKE') ? 'TSK-INTAKE-SMOKE' : 'TSK-STAGING1',
            title: 'smoke',
    ...
    if (route.includes('/github/webhooks') || route.includes('/gitlab/webhooks')) {
      return { ok: true, status: 201, json: async () => ({ taskId: 'TSK-INTAKE1', task_id: 'TSK-INTAKE1' }) };
    }
  ...
  try {
    ...
    assert.ok(requests.some((entry) => (
      entry.url.includes('/github/webhooks') || entry.url.includes('/gitlab/webhooks')
    )));
  } finally {
