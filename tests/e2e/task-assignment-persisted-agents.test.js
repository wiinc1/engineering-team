...
test("e2e: supported persisted agent assigns live then deactivates without losing historical owner display", async () => {
  await withServer(async ({ baseUrl, secret }) => {
    ...
    const createdVersion = body.data.version;

    response = await fetch(`${baseUrl}/ai-agents`, {
      headers: authHeaders(secret, ["reader"]),
    });
    assert.equal(response.status, 200);
    body = await response.json();
    assert.equal(
      body.items.some((agent) => agent.id === agentId && agent.display_name === "Persisted E2E QA"),
      true,
    );

    response = await fetch(`${baseUrl}/tasks/${taskId}/events`, {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        payload: {
          title: "Persisted assignment owner display",
    ...
    response = await fetch(`${baseUrl}/tasks/${taskId}/assignment`, {
      method: "PATCH",
      headers: { "content-type": "application/json", ...authHeaders(secret, ["pm"]) },
    ...
    response = await fetch(`${baseUrl}/api/v1/ai-agents/${agentId}`, {
      method: "PATCH",
      headers: { "content-type": "application/json", ...authHeaders(secret, ["pm"]) },
    ...
    body = await response.json();
    assert.equal(
      body.items.some((agent) => agent.id === agentId),
      false,
    );
    ...
    body = await response.json();
    assert.equal(
      body.data.some((agent) => agent.agentId === agentId && agent.active === false && agent.assignable === false),
      true,
    );
